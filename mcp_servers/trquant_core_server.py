...
try:
    from utils.m33_tools import M33_TOOLS, M33_HANDLERS
    
    for tool in M33_TOOLS:
        TOOLS.append(tool)
        TOOL_HANDLERS[tool.name] = M33_HANDLERS.get(tool.name)
    
    M33_INTEGRATED = True
    logger.info(f"M3.3工具已集成: {len(M33_TOOLS)} 个")
except ImportError as e:
    M33_INTEGRATED = False
    logger.warning(f"M3.3工具集成失败: {e}")

# ==================== Strategy Pack工具集成 ====================

try:
    from utils.strategy_tools import STRATEGY_TOOLS, STRATEGY_HANDLERS
    
    for tool in STRATEGY_TOOLS:
        TOOLS.append(tool)
        TOOL_HANDLERS[tool.name] = STRATEGY_HANDLERS.get(tool.name)
    
    STRATEGY_INTEGRATED = True
    logger.info(f"Strategy Pack工具已集成: {len(STRATEGY_TOOLS)} 个")
except ImportError as e:
    STRATEGY_INTEGRATED = False
    logger.warning(f"Strategy Pack工具集成失败: {e}")

# ==================== Tier2 AltData工具集成 ====================

try:
    from utils.altdata_tools import ALTDATA_TOOLS, ALTDATA_HANDLERS
    
    for tool in ALTDATA_TOOLS:
        TOOLS.append(tool)
        TOOL_HANDLERS[tool.name] = ALTDATA_HANDLERS.get(tool.name)
    
    ALTDATA_INTEGRATED = True
    logger.info(f"Tier2 AltData工具已集成: {len(ALTDATA_TOOLS)} 个")
except ImportError as e:
    ALTDATA_INTEGRATED = False
    logger.warning(f"Tier2 AltData工具集成失败: {e}")
...
try:
    ...
    TENBAGGER_INTEGRATED = True
    logger.info(f"Tenbagger评估工具已集成: {len(TENBAGGER_TOOLS)} 个")
except ImportError as e:
    TENBAGGER_INTEGRATED = False
    logger.warning(f"Tenbagger评估工具集成失败: {e}")
...
try:
    from utils.portfolio_tools import PORTFOLIO_TOOLS, PORTFOLIO_HANDLERS
    
    for tool in PORTFOLIO_TOOLS:
        TOOLS.append(tool)
        TOOL_HANDLERS[tool.name] = PORTFOLIO_HANDLERS.get(tool.name)
    
    PORTFOLIO_INTEGRATED = True
    logger.info(f"Portfolio组合管理工具已集成: {len(PORTFOLIO_TOOLS)} 个")
except ImportError as e:
    PORTFOLIO_INTEGRATED = False
    logger.warning(f"Portfolio组合管理工具集成失败: {e}")

# ==================== DataSource数据源工具集成 ====================

try:
    from utils.datasource_tools import DATASOURCE_TOOLS, DATASOURCE_HANDLERS
    
    for tool in DATASOURCE_TOOLS:
        TOOLS.append(tool)
        TOOL_HANDLERS[tool.name] = DATASOURCE_HANDLERS.get(tool.name)
    
    DATASOURCE_INTEGRATED = True
    logger.info(f"DataSource数据源工具已集成: {len(DATASOURCE_TOOLS)} 个")
except ImportError as e:
    DATASOURCE_INTEGRATED = False
    logger.warning(f"DataSource数据源工具集成失败: {e}")
