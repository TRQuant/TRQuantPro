...
# ==================== 补充工具: event.stage_mapping ====================

def _handle_event_stage_mapping(**kwargs) -> dict:
    """获取事件→阶段映射表"""
    from .event_extractor import EventType, STAGE_IMPACT, EventExtractor
    
    mappings = []
    rules = EventExtractor.EXTRACTION_RULES
    
    for event_type in EventType:
        stage_impact = STAGE_IMPACT.get(event_type, (None, None))
        keywords = []
        if event_type in rules:
            keywords = rules[event_type].get("keywords", [])[:3]
        
        mappings.append({
            "event_type": event_type.value,
            "event_name": event_type.name,
            "from_stage": stage_impact[0],
            "to_stage": stage_impact[1],
            "keywords": keywords
        })
...
# 添加到工具列表
M31_TOOLS.append({
    "name": "event.stage_mapping",
    "description": "获取事件→阶段映射表，包含每种事件触发的阶段转换",
    "handler": _handle_event_stage_mapping
})
