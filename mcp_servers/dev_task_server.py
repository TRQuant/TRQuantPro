...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    logger.info(f"调用工具: {original_name} -> {name}")
    
    # 调试：确认转换后的name值
    if original_name != name:
        logger.info(f"工具名称已转换: {original_name} -> {name}")
    ...
    try:
        ...
        # 批量执行工具
        elif name == "workflow.batch":
            tools = arguments.get("tools", [])
            stop_on_input = arguments.get("stop_on_input", True)
            stop_on_error = arguments.get("stop_on_error", False)
            max_calls = arguments.get("max_calls", 50)
            ...
            for i, tool_config in enumerate(tools):
                ...
                try:
                    tool_result_list = await call_tool(tool_name, tool_args)
                    tool_result = json.loads(tool_result_list[0].text) if tool_result_list else {}
                    
                    needs_input = (
                        tool_result.get("need_input") is True or
                        tool_result.get("requires_user_decision") is True or
                        tool_result.get("status") == "pending_user_input"
                    )
                    
                    batch_results.append({
                        "index": i,
                        "tool_name": tool_name,
                        "status": "need_input" if needs_input else "success",
                        "result": tool_result
                    })
                    
                    if needs_input and stop_on_input:
                        status = "need_input"
                        stopped_at = i
                        break
            ...
            result = {
                ...
                "successful_calls": sum(1 for r in batch_results if r.get("status") == "success"),
                "stopped_at": stopped_at,
                "results": batch_results,
                "message": f"批量执行完成: {sum(1 for r in batch_results if r.get('status') == 'success')}/{len(batch_results)} 成功"
            }
        
        elif name == "workflow.auto":
            ...
            for i, tool_config in enumerate(tools):
                ...
                for key, value in tool_args.items():
                    if isinstance(value, str) and value.startswith("$"):
                        context_key = value[1:]
                        tool_args[key] = context.get(context_key, value)
                ...
                try:
                    ...
                    if isinstance(tool_result, dict) and "context" in tool_result:
                        context.update(tool_result["context"])
                    
                    needs_input = (
                        tool_result.get("need_input") is True or
                        tool_result.get("requires_user_decision") is True
                    )
            ...
            result = {
                ...
                "context": context,
                "results": batch_results,
                "message": f"自动执行完成: {sum(1 for r in batch_results if r.get('status') == 'success')}/{len(batch_results)} 成功"
            }
            if status == "need_input":
                result["message"] += "，已停止等待用户输入"
