...
async def _handle_get_realtime(args: Dict) -> Dict:
    ...
    try:
        ...
        if alltick.connect():
            results = {}
            for symbol in args["securities"]:
                price_info = alltick.get_realtime_price(symbol)
                if price_info:
                    results[symbol] = {
                        "price": price_info["price"],
                        "volume": price_info.get("volume", 0),
                        "timestamp": price_info["timestamp"].isoformat() if hasattr(price_info["timestamp"], "isoformat") else str(price_info["timestamp"])
                    }
            if results:
                return {"success": True, "source": "alltick", "data": results}
    except Exception as e:
        import logging
        logging.getLogger(__name__).warning(f"AllTick获取失败，降级到AKShare: {e}")
    ...
    try:
        ...
        if akshare.available:
            ...
            if not df.empty:
                return {"success": True, "source": "akshare", "count": len(df), "data": df.to_dict(orient="records")}
    except:
        pass
...
async def _handle_candidate_pool(args: Dict) -> Dict:
    ...
    mainline_type = args.get("mainline_type", "concept")
    ...
    try:
        ...
        # 转换为标准格式
        stocks = []
        for stock in pool.stocks[:limit]:
        ...
        return {
            ...
            "mainline_type": mainline_type,
            "stocks": stocks,
            "candidates": stocks,  # 兼容字段
            "pool_size": len(stocks),
            "total_count": pool.total_count,
            "filtered_count": pool.filtered_count,
