...
async def _handle_mainlines(args: Dict) -> Dict:
    """获取市场主线 - 使用WorkflowOrchestrator（直接调用AKShare）"""
    ...
    try:
        ...
        if result.success:
            mainlines = result.details.get('top_mainlines', result.details.get('mainlines', []))
            
            # 如果需要详细的五维评分
            if include_detail and mainlines:
                try:
                    ...
                    if sector_result.success or concept_result.success:
                        five_dim_results = engine.calculate(
                        ...
                        # 匹配五维评分到主线
                        score_map = {r.name: r for r in five_dim_results}
                        for ml in mainlines:
                            name = ml.get('name', '')
                            if name in score_map:
                                ...
                                ml['five_dim_score'] = {
                                    'total': r.total_score,
                                    'funds': r.funds_score.score if r.funds_score else 0,
                                    'heat': r.heat_score.score if r.heat_score else 0,
                                    'momentum': r.momentum_score.score if r.momentum_score else 0,
                                    'policy': r.policy_score.score if r.policy_score else 0,
                                    'leader': r.leader_score.score if r.leader_score else 0,
                                }
                except Exception as e:
            ...
            return {
                ...
                "data_source": result.details.get('data_source', 'akshare'),
                "update_time": result.timestamp
            }
        else:
            return {
                "success": False,
                "error": result.error or "主线识别失败",
                "count": 0,
                "mainlines": []
            }
    except Exception as e:
        logger.error(f"主线识别异常: {e}", exc_info=True)
        return {
            "success": False,
            "error": f"主线识别异常: {str(e)}",
            "count": 0,
            "mainlines": []
        }
