...
TOOLS = [
    ...
    Tool(
        ...
        description="增强回测 - 完整指标计算（夏普/索提诺/卡玛/Beta/Alpha），支持因子分析和ML",
        inputSchema={
            "type": "object",
            "properties": {
                ...
                "commission_rate": {"type": "number", "description": "佣金率（万一=0.0001）", "default": 0.0001},
                "enable_factor_analysis": {"type": "boolean", "description": "启用因子分析", "default": True},
                "enable_ml": {"type": "boolean", "description": "启用机器学习", "default": False}
            },
            "required": ["securities", "start_date", "end_date"]
        }
    ),
    # 十倍股回测（新增）
    Tool(
        name="backtest.tenbagger",
        description="十倍股策略回测 - 基于多因子模型，目标高收益",
        inputSchema={
            "type": "object",
            "properties": {
                "start_date": {"type": "string", "description": "开始日期 YYYY-MM-DD"},
                ...
                "max_holdings": {"type": "integer", "description": "最大持仓数", "default": 5},
                "min_score": {"type": "number", "description": "最低得分", "default": 75},
                "stop_loss": {"type": "number", "description": "止损比例", "default": -0.15},
                "take_profit": {"type": "number", "description": "止盈比例", "default": 1.5}
            },
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    try:
        ...
        elif name == "backtest.enhanced":
            result = await _handle_enhanced(arguments)
        elif name == "backtest.tenbagger":
            result = await _handle_tenbagger(arguments)
...
async def _handle_enhanced(args: Dict) -> Dict:
    """增强回测 - 完整指标"""
    ...
    config = EnhancedBacktestConfig(
        ...
        initial_capital=args.get("initial_capital", 1000000),
        commission_rate=args.get("commission_rate", 0.0001),  # 万一佣金
        enable_factor_analysis=args.get("enable_factor_analysis", True),
        enable_ml=args.get("enable_ml", False)
    )
    ...
    return {
        ...
        "metrics": {
            ...
            "info_ratio": round(result.info_ratio, 2),
            "win_rate": f"{result.win_rate*100:.1f}%",
...
async def _handle_tenbagger(args: Dict) -> Dict:
    """十倍股策略回测"""
    from core.backtest.enhanced_backtest import (
        EnhancedBacktestEngine,
        EnhancedBacktestConfig
    ...
    config = EnhancedBacktestConfig(
        ...
        max_holdings=args.get("max_holdings", 5),
        stop_loss=args.get("stop_loss", -0.15),
        take_profit=args.get("take_profit", 1.5),
    ...
    # 获取中证500+创业板作为股票池
    try:
        securities = jq.get_index_stocks('000905.XSHG')
        securities += jq.get_index_stocks('399006.XSHE')[:100]
    ...
    return {
        ...
        "parameters": {
            ...
            "stop_loss": f"{config.stop_loss*100:.0f}%",
            "take_profit": f"{config.take_profit*100:.0f}%",
            "stock_pool_size": len(config.securities)
        },
        "metrics": {
            "total_return": f"{result.total_return*100:.2f}%",
            "annual_return": f"{result.annual_return*100:.2f}%",
