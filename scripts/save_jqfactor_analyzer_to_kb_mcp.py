...
# -*- coding: utf-8 -*-
"""
使用MCP工具将jqfactor-analyzer信息存入向量RAG知识库
"""
...
def create_jqfactor_analyzer_kb_items() -> list:
...
def main():
    """主函数 - 使用MCP工具添加知识库"""
    print("=" * 70)
    print("使用MCP工具存入jqfactor-analyzer知识库")
    print("=" * 70)
            ...
            if mcp_result.success:
                ...
                if isinstance(result_data, dict):
                    if result_data.get('success') or result_data.get('knowledge_id'):
                        results['success'] += 1
                        kb_id = result_data.get('knowledge_id') or result_data.get('id') or 'unknown'
