...
# -*- coding: utf-8 -*-
"""
BulletTrade 回测验证脚本（最终修复版本）
========================================

使用LimitOrder提高限价，确保订单能够成交
"""
...
def main():
    print("=" * 70)
    print("🚀 BulletTrade 回测验证（使用LimitOrder提高限价）")
    print("=" * 70)
    ...
    try:
        ...
        # 3. 创建策略代码（使用LimitOrder提高限价）
        print("\n📝 步骤3: 创建策略代码（使用LimitOrder提高限价）...")
        strategy_code = '''
# 买入持有策略（使用LimitOrder提高限价确保成交）
    ...
    if not context.bought:
        ...
        for stock in context.stocks:
            ...
            if stock in current_data:
                ...
                # 使用LimitOrder，设置限价为当前价格的105%（确保能够成交）
                limit_price = current_price * 1.05
                order_target_value(stock, target_value_per_stock, style=LimitOrder(price=limit_price))
                log.info(f'[买入] 日期: {current_date}, 股票: {stock}, 目标价值: {target_value_per_stock:.0f}, 限价: {limit_price:.2f}')
            else:
            ...
            for stock in context.stocks:
                ...
                if stock in context.portfolio.positions:
                    ...
                    if hasattr(position, 'total_amount') and position.total_amount > 0:
                        # 获取当前价格，使用LimitOrder设置限价为当前价格的95%（确保能够成交）
                        if stock in current_data:
                            current_price = current_data[stock].last_price
                            limit_price = current_price * 0.95
                            order_target_value(stock, 0, style=LimitOrder(price=limit_price))
                            log.info(f'[卖出] 日期: {current_date}, 股票: {stock}, 持有天数: {days_held}, 限价: {limit_price:.2f}')
            
            context.buy_date = None
'''
        print("   ✅ 策略代码已创建（使用LimitOrder提高限价）")
        ...
        # 6. 执行回测
        print("\n⏳ 步骤6: 执行回测...")
        output_dir = PROJECT_ROOT / 'backtest_results' / 'test_final_fixed'
