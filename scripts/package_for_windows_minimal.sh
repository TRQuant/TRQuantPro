#!/bin/bash
# TRQuant Windows迁移打包脚本（最小化版本）
# 用途: 只打包必需文件，排除可重建内容，大幅减少文件大小
# 作者: TRQuant Team
# 日期: 2026-01-11
...
echo -e "${GREEN}🚀 TRQuant Windows迁移打包工具（最小化版本）${NC}"
echo "=========================================="
echo -e "${YELLOW}⚠️  重要: 此脚本从ope目录打包，将安装到Windows的 C:\\TRQuantPro\\ope${NC}"
echo -e "${BLUE}📦 优化策略: 排除可重建文件（venv、缓存、数据、向量索引等）${NC}"
echo ""
...
# 生成包名
PACKAGE_NAME="TRQuant_Windows_Minimal_$(date +%Y%m%d_%H%M%S).tar.gz"
PACKAGE_DIR="TRQuant_Windows_Package_Minimal_$(date +%Y%m%d_%H%M%S)"
...
# MCP服务器
if [ -d "$PROJECT_ROOT/mcp_servers" ]; then
    echo "  ✅ mcp_servers/ - MCP工具接口"
    rsync -av --exclude='__pycache__' --exclude='*.pyc' --exclude='*.pyo' \
...
if [ -d "$PROJECT_ROOT/notebooks" ]; then
    echo "  ✅ notebooks/ - Jupyter Notebook研究前端"
...
if [ -d "$PROJECT_ROOT/strategies" ]; then
    echo "  ✅ strategies/ - 策略文件"
    rsync -av --exclude='__pycache__' --exclude='*.pyc' \
        "$PROJECT_ROOT/strategies/" TRQuant/strategies/ > /dev/null 2>&1 || true
fi

# 数据源模块
if [ -d "$PROJECT_ROOT/data_sources" ]; then
    echo "  ✅ data_sources/ - 数据源模块"
    rsync -av --exclude='__pycache__' --exclude='*.pyc' \
...
if [ -d "$PROJECT_ROOT/config" ]; then
    echo "  ✅ config/ - 配置文件（模板）"
    rsync -av "$PROJECT_ROOT/config/" TRQuant/config/ > /dev/null 2>&1 || true
    # 创建配置模板
...
if [ -d "$PROJECT_ROOT/.trquant/dev/knowledge/vector_index" ]; then
    ...
    echo "  ⚠️  vector_index/ 存在 (${VECTOR_INDEX_SIZE})，但可以重建，建议排除以减小包大小"
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        echo "  ⏭️  跳过向量索引（将在Windows上重建）"
    fi
fi

# ==================== 可选模块 ====================
echo ""
echo -e "${YELLOW}📦 可选模块（根据需要）...${NC}"
...
if [ -f "$PROJECT_ROOT/README_WINDOWS_MIGRATION.md" ]; then
    cp "$PROJECT_ROOT/README_WINDOWS_MIGRATION.md" TRQuant/ 2>/dev/null || true
    echo "  ✅ README_WINDOWS_MIGRATION.md"
fi
...
# ==================== 创建Windows安装脚本 ====================
echo ""
echo -e "${GREEN}🔧 创建Windows安装脚本...${NC}"

cat > TRQuant/install_windows.ps1 << 'INSTALL_EOF'
# TRQuant Windows安装脚本（最小化版本）
...
Write-Host "🚀 TRQuant Windows安装程序（最小化版本）" -ForegroundColor Green
...
# 创建启动脚本
cat > TRQuant/start_jupyter.bat << 'BAT_EOF'
@echo off
...
# 创建测试连接脚本
cat > TRQuant/test_connection.bat << 'BAT_EOF'
@echo off
REM TRQuant连接测试脚本
REM 安装路径: C:\TRQuantPro\ope
cd /d C:\TRQuantPro\ope
...
# 创建README
cat > TRQuant/README_WINDOWS.md << 'README_EOF'
# TRQuant Windows安装说明（最小化版本）

## ⚠️ 重要提示

- **安装路径**: `C:\TRQuantPro\ope` (与Ubuntu结构一致)
- **Python版本**: 3.11 或 3.10 (QMT需要3.12以下)
- **知识库**: 已包含JSON文件，向量索引可在Windows上重建

## 快速安装

1. **解压文件**
   - 将 `TRQuant_Windows_Minimal_*.tar.gz` 解压到临时目录
...
- ✅ **Python虚拟环境** (`venv/`) - 通过 `install_windows.ps1` 自动创建
- ✅ **Python库** - 通过 `pip install -r requirements.txt` 安装
- ✅ **知识库向量索引** - 运行 `python scripts\kb\kb_manager.py build-index`
- ✅ **数据文件** - 首次运行时自动下载
- ✅ **缓存文件** - 运行时自动生成
...
知识库JSON文件已包含在打包中：
- `knowledge_base.json` - 知识库JSON文件 ✅ 已包含
- `vector_index/` - ChromaDB向量索引 ⚠️ 可选（可重建）
...
echo "  ✅ 核心代码 (core, mcp_servers, notebooks, scripts, strategies)"
echo "  ✅ 配置文件 (config/)"
