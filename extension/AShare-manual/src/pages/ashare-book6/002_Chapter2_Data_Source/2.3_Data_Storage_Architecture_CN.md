...
> **核心摘要：**
> 
> 本节详细介绍数据源模块的数据存储架构，采用**分层/多存储（Polyglot Persistence）**架构，根据数据类型选择最适合的存储方案。通过理解PostgreSQL主库、ClickHouse/TimescaleDB时序库、Redis缓存等技术的选型理由和数据分布策略，帮助开发者掌握数据源模块的数据存储核心设计。
...
- **理解架构理念**：掌握分层/多存储架构的核心思想
- **熟悉PostgreSQL存储**：了解数据源配置和元数据的存储设计
- **掌握时序库存储**：理解行情数据和因子数据的存储策略
- **使用Redis缓存**：掌握缓存策略和任务队列的实现
- **理解文件缓存**：了解Parquet/HDF5文件缓存的使用场景
- **掌握数据流转**：理解数据从获取到存储到查询的完整流程

<h2 id="section-2-3-1">🎯 2.3.1 架构设计理念</h2>

### 核心理念：分层/多存储（Polyglot Persistence）

数据源模块采用**分层/多存储（Polyglot Persistence）**架构，根据数据类型的特点选择最适合的存储方案，而不是"单库打天下"。
...
<div class="key-points">
  <div class="key-point">
    <h4>📊 按数据类型选择存储</h4>
    <p>强事务数据用PostgreSQL，时序数据用时序库，临时数据用Redis</p>
  </div>
  <div class="key-point">
    <h4>⚡ 性能优先</h4>
    <p>每种数据使用最适合的存储，获得最佳性能</p>
  </div>
  <div class="key-point">
    <h4>💰 成本优化</h4>
    <p>在满足性能要求的前提下，优化存储成本</p>
  </div>
  <div class="key-point">
    <h4>🔄 可扩展性</h4>
    <p>支持水平扩展，提高系统容量</p>
...
<div class="comparison-table">
  <table>
    ...
    <tbody>
      <tr>
        <td><strong>数据源配置</strong></td>
        <td>PostgreSQL</td>
        <td>数据源连接配置、元数据、审计日志</td>
        <td>强事务、强一致性、可审计</td>
      </tr>
      <tr>
        <td><strong>行情数据</strong></td>
        <td>ClickHouse/TimescaleDB</td>
        <td>OHLCV日/分钟/Tick数据</td>
        <td>高效聚合、时间序列查询</td>
      </tr>
      <tr>
        <td><strong>因子数据</strong></td>
        <td>ClickHouse/TimescaleDB</td>
        <td>每日/每分钟因子矩阵</td>
        <td>列式存储、压缩率高</td>
      </tr>
      <tr>
        <td><strong>行情快照</strong></td>
        <td>Redis</td>
        <td>实时行情数据缓存（TTL: 60秒）</td>
        <td>高速访问、低延迟</td>
      </tr>
      <tr>
...
<h2 id="section-2-3-2">🗄️ 2.3.2 PostgreSQL存储</h2>

### 存储内容

PostgreSQL用于存储数据源模块的**强事务/强审计类数据**：

1. **数据源配置**：数据源连接配置、认证信息
2. **元数据**：数据源状态、健康检查记录、使用统计
3. **审计日志**：数据获取记录、错误日志、操作历史
...
CREATE TABLE data_source_metadata (
    ...
    completeness DECIMAL(5,2), -- 完整性（0-100）
    accuracy DECIMAL(5,2), -- 准确性（0-100）
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE data_fetch_logs (
    ...
    fetch_duration_ms INTEGER, -- 获取耗时（毫秒）
...
<CodeFromFile 
  filePath="code_library/002_Chapter2_Data_Source/2.3/002_Chapter2_Data_Source/2.3/code_2_3_00.py"
  language="python"
...
<!-- 原始代码（保留作为备份）：
<CodeFromFile 
  filePath="code_library/002_Chapter2_Data_Source/2.3/code_2_3_00.py"
  language="python"
  showDesignPrinciples="true"
/>
...
<h2 id="section-2-3-3">⏱️ 2.3.3 时序库存储</h2>

### 存储内容

时序数据库（ClickHouse或TimescaleDB）用于存储数据源模块的**时间序列数据**：

1. **行情数据**：OHLCV日线、分钟线、Tick数据
2. **因子数据**：每日/每分钟因子矩阵
3. **回测输出**：净值曲线、分层归因、行业暴露
...
CREATE TABLE market_data_daily (
    ...
    volume UInt64,
    amount Decimal64(2),
    turnover_rate Decimal64(4), -- 换手率
    pe_ratio Decimal64(4), -- 市盈率
    pb_ratio Decimal64(4), -- 市净率
    source String, -- 数据源名称
    created_at DateTime DEFAULT now()
) ENGINE = MergeTree()
PARTITION BY toYYYYMM(trade_date)
ORDER BY (symbol, trade_date)
TTL trade_date + INTERVAL 10 YEAR; -- 保留10年
...
TTL trade_time + INTERVAL 2 YEAR; -- 保留2年
```
...
如果选择TimescaleDB，可以使用PostgreSQL的时序扩展：
...
-- <CodeFromFile 
  filePath="code_library/<CodeFromFi<CodeFromFi<CodeFromFi<CodeFromFile 
  filePath="code_library/002_Chapter2_Data_Source/2.3/code_2_3_store_market_data.py"
  language="python"
  showDesignPrinciples="true"
/>
...
def dequeue_data_update_task() -> dict:
    ...
    tasks = r.zrevrange(<CodeFromFile 
  <CodeFromFile 
  filePath="code_library/002_Chapter2_Data_Source/2.<CodeFromFile 
  filePath="code_library/002_Chapter2_Data_Source/2.3/code_2_3_save_to_hdf5.py"
  language="python"
  showDesignPrinciples="true"
/>
...
def save_to_hdf5(symbol: str, data: pd.DataFrame, da<CodeFromFile 
  filePath="code_librar<CodeFromFile 
  filePath="code_library/002_Chapter2_Data_Source/2.3/code_2_3_save_to_hdf5.py"
  language="python"
  showDesignPrinciples="true"
/>
...
<!-- 原始代码（保留作为备份）：
```
-->ata_type: str = "daily") -> pd.Dat<CodeFromFile 
  fileP<CodeFromFile 
  filePath="code_library/002_Chapter2_Data_Source/2.3/code_2_3_save_to_hdf5.py"
  language="python"
  showDesignPrinciples="true"
...
class DataStorageManager:
    ...
    def get_data(self, symbol: str, start_date: str, end_date: str, 
        ...
        return pd.DataFrame()
```
-->ol, start_date, end_date, data_type)
...
2. **PostgreSQL存储**：数据源配置、元数据、审计日志
3. **时序库存储**：行情数据、因子数据、回测输出
4. **Redis缓存**：行情快照、任务队列、状态缓存
5. **文件缓存**：Parquet/HDF5适合早期阶段或小规模数据
6. **数据流转**：缓存优先 → 时序库查询 → 数据源获取 → 存储

---

*最后更新: 2025-12-13*  
*适用版本: v1.0.0+*
<!-- Code updated: 2025-12-14T01:36:03.391Z -->
