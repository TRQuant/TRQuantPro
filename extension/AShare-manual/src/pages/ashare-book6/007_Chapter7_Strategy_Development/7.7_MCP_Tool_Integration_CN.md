...
MCPTool(
    ...
    input_schema={
        ...
        "required": ["platform", "style", "factors"]
    }
)
```
...
# 返回结果示例
{
    ...
    "style": "multi_factor",
    "factors": ["factor_pe", "factor_roe", "factor_momentum"],
    "file_path": "strategies/ptrade/multi_factor_ptrade_20251212_120000.py"
}
```
...
# 返回结果示例
{
    ...
    "recommendations": [
        "建议优化因子权重配置",
        "考虑增加止损止盈机制",
        "建议进行Walk-Forward分析验证稳健性"
    ]
}
```

<h2 id="section-7-7-2">🔍 7.7.2 KB MCP Server工具</h2>

KB MCP Server提供知识库查询能力，支持查询开发手册和工程代码库。

### kb.query

查询知识库，获取策略开发相关的文档和代码。
...
<h2 id="section-7-7-3">🔄 7.7.3 工具集成工作流</h2>

MCP工具在策略开发中的完整工作流和最佳实践。
