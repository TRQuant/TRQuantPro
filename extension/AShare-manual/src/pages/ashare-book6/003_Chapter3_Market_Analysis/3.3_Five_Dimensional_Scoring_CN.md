...
<h2 id="section-3-3-2">💰 3.3.2 资金维度评分</h2>

资金维度评分通过分析资金流向指标，评估市场资金面的强弱。
...
def score_capital_dimension(northbound_flow: float, margin_balance: float, 
    ...
    if fund_position > 85:
        ...
        fund_score = max(0, (fund_position - 60) * 0.5)
    
    score = northbound_score + margin_score + fund_score
    return min(100, max(0, score))
```
...
<h2 id="section-3-3-3">🏭 3.3.3 行业维度评分</h2>

行业维度评分通过分析行业轮动、行业景气度、行业估值等指标，评估行业层面的市场环境。
...
<h2 id="section-3-3-4">📈 3.3.4 技术维度评分</h2>

技术维度评分通过分析指数趋势、技术形态、技术指标等，评估市场技术面的强弱。
...
<h2 id="section-3-3-5">💎 3.3.5 估值维度评分</h2>

估值维度评分通过分析市场估值水平、行业估值、个股估值等，评估市场估值是否合理。
...
def score_valuation_dimension(market_valuation: float, industry_valuation: float, 
    ...
    score = market_score + industry_score + stock_score
    return min(100, max(0, score))
```

<h2 id="section-3-3-6">🔢 3.3.6 综合评分计算</h2>

综合评分通过加权平均计算各维度评分，得到最终的市场环境评分。
...
DEFAULT_WEIGHTS = {
    ...
    'capital': 0.25,    # 资金维度权重：25%
    'industry': 0.20,   # 行业维度权重：20%
    'technical': 0.20,  # 技术维度权重：20%
    'valuation': 0.15   # 估值维度权重：15%
}
```
