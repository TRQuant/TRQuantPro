    ...
    private executeSubprocess<T>(
        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.extensionPath.endsWith("extension") || this.extensionPath.endsWith("extension/")) {
                ...
                // 如果extensionPath是安装目录，尝试从环境变量获取或硬编码路径
                const hardcodedRoot = '/home/taotao/dev/QuantTest/TRQuant';
                if (fs.existsSync(hardcodedRoot)) {
                    trquantRoot = hardcodedRoot;
                } else {
                    trquantRoot = process.env.TRQUANT_ROOT || trquantRoot;
                }
            }
            
            // 设置环境变量 - PYTHONPATH需要包含项目根目录和extension/python
            const env = {
                ...process.env,
                PYTHONIOENCODING: 'utf-8',
                TRQUANT_ROOT: trquantRoot,
                PYTHONPATH: [
                    trquantRoot,  // 项目根目录（用于导入core模块）
                    path.join(trquantRoot, 'mcp_servers'),  // MCP服务器目录
                    path.join(this.extensionPath, 'python')  // bridge.py所在目录
                ].join(path.delimiter)
            };

            const proc = cp.spawn(pythonPath, [bridgePath], {
                cwd: trquantRoot,  // 工作目录设置为项目根目录
                env,
