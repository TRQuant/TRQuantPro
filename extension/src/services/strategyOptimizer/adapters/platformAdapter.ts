...
import { Platform, APIMapping, ConversionResult, CodeChange, StrategyAnalysis } from '../types';
...
const API_MAPPINGS: APIMapping[] = [
  // ============ 数据获取 ============
  {
    ...
    target: {
      platform: 'ptrade',
      api: 'get_history',
      signature: 'get_history(count, frequency, field, security_list, skip_paused, fq, bar_count)',
    },
    paramMapping: {
  ...
  },
  {
    source: {
      ...
      signature: 'get_current_data(security)',
    },
  ...
  },
  {
    source: {
      platform: 'joinquant',
      api: 'history',
      signature: 'history(count, unit, field, security_list, df, skip_paused, fq)',
    },
    target: {
      platform: 'ptrade',
      api: 'get_history',
  ...
  },
  {
    source: {
      platform: 'joinquant',
      api: 'get_index_stocks',
      signature: 'get_index_stocks(index_symbol, date)',
    },
    target: {
      platform: 'ptrade',
      api: 'get_index_stocks',
  ...
  // ============ 交易执行 ============
  {
    source: {
      ...
      signature: 'order_target_value(security, value)',
    },
    target: {
      platform: 'ptrade',
      api: 'order_target_percent',
      signature: 'order_target_percent(stock_code, percent)',
    },
    paramMapping: {
      security: 'stock_code',
  ...
  },
  {
    ...
    target: {
      platform: 'ptrade',
      api: 'order_lots',
      signature: 'order_lots(stock_code, lots, limit_price)',
    },
    paramMapping: {
      security: 'stock_code',
  ...
  },
  {
    source: {
      platform: 'joinquant',
      api: 'order_target',
      signature: 'order_target(security, amount)',
    },
    target: {
      platform: 'ptrade',
      api: 'order_target_volume',
      signature: 'order_target_volume(stock_code, volume)',
    },
    paramMapping: {
      security: 'stock_code',
      amount: 'volume',
  ...
  },
  {
    source: {
      platform: 'joinquant',
      api: 'set_option',
      signature: 'set_option(option_name, value)',
    },
    target: { platform: 'ptrade', api: '', signature: '' },
    paramMapping: {},
  ...
  },
  {
    source: {
      platform: 'joinquant',
      api: 'set_order_cost',
      signature: 'set_order_cost(cost, type)',
    },
    target: { platform: 'ptrade', api: '', signature: '' },
    paramMapping: {},
...
export class PlatformAdapter {
   ...
   * JoinQuant → PTrade 转换
   */
  private convertJoinQuantToPTrade(code: string, analysis: StrategyAnalysis): ConversionResult {
    const changes: CodeChange[] = [];
    const warnings: string[] = [];
    const errors: string[] = [];
...
`;
    ...
    // 5. 处理run_daily
    const runDailyMatches = code.matchAll(
      /run_daily\s*\(\s*(\w+)\s*,\s*['"](\d{2}:\d{2})['"]\s*\)/g
    );
