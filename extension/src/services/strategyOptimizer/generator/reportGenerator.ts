...
export class ReportGenerator {
   ...
   * 生成完整策略报告
   */
  generate(analysis: StrategyAnalysis, conversionResult?: ConversionResult): StrategyReport {
    return {
   ...
   * 生成Markdown格式报告
   */
  generateMarkdown(analysis: StrategyAnalysis, conversionResult?: ConversionResult): string {
...
${report.codeArchitecture.modules
  .map(
    (m) => `
#### ${m.name}
- **用途**: ${m.purpose}
...
| 平台 | 兼容度 | 等级 | 需修改 | 备注 |
|------|--------|------|--------|------|
${Object.entries(report.platformCompatibility.details)
  .map(
    ([platform, info]) =>
...
`;
   ...
   * 生成HTML格式报告
   */
  generateHTML(analysis: StrategyAnalysis, conversionResult?: ConversionResult): string {
    const report = this.generate(analysis, conversionResult);
...
${report.codeArchitecture.flowChart}
        ...
        <div class="card">
            ...
            <div class="grid">
                ${Object.entries(report.codeQuality.metrics)
                  .map(
                    ([key, value]) => `
                <div>
  ...
  private generatePhilosophy(analysis: StrategyAnalysis): StrategyReport['investmentPhilosophy'] {
    ...
    if (analysis.dependencies.indicators.length > 0) {
      uniqueFeatures.push(
        `技术指标辅助（${analysis.dependencies.indicators.map((i) => i.name).join('、')}）`
      );
    }
  ...
  private generateCompatibility(
    ...
    const summary =
      `当前代码基于 ${sourcePlatform.toUpperCase()} 平台开发。` +
  ...
  private generateQuality(analysis: StrategyAnalysis): StrategyReport['codeQuality'] {
    ...
    // 可维护性
    metrics.maintainability = Math.min(
      100,
      Math.max(
        0,
        100 -
          analysis.stats.totalLines / 10 +
          analysis.stats.functionCount * 5 -
          (analysis.stats.complexity === 'high'
            ? 20
            : analysis.stats.complexity === 'medium'
              ? 10
              : 0)
      )
    );
    ...
    // 可测试性
    metrics.testability = Math.min(
      100,
      analysis.stats.functionCount * 10 + analysis.components.utilities.length * 5
    );
    ...
    // 复杂度控制
    metrics.complexity =
      analysis.stats.complexity === 'low' ? 90 : analysis.stats.complexity === 'medium' ? 70 : 50;

    const score = Math.round(
      (metrics.maintainability + metrics.testability + metrics.documentation + metrics.complexity) /
        4
    );
