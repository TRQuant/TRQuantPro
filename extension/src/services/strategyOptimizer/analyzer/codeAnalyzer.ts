...
export class CodeAnalyzer {
  ...
  private extractMeta(code: string, filename?: string): StrategyAnalysis['meta'] {
    ...
    if (docstringMatch) {
      ...
      // 提取回测区间
      const backtestMatch = docstring.match(
        /回测区间[：:]\s*(\d{4}-\d{2}-\d{2})\s*至\s*(\d{4}-\d{2}-\d{2})/
      );
      if (backtestMatch) {
        meta.backtestPeriod = {
  ...
  private categorizeComponents(blocks: CodeBlock[]): StrategyAnalysis['components'] {
    ...
    for (const block of blocks) {
      ...
      if (
        ...
        name.includes('stock') ||
        content.includes('选股') ||
        content.includes('stock_pool') ||
        content.includes('get_index_stocks')
      ) {
  ...
  private hasAPIMapping(apiName: string, source: Platform, target: Platform): boolean {
    ...
    const mappings: Record<string, PlatformMapping> = {
      get_price: {
        joinquant: 'get_price',
        ptrade: 'get_history',
        qmt: 'ContextInfo.get_market_data',
      },
      get_current_data: {
        joinquant: 'get_current_data',
        ptrade: 'get_snapshot',
        qmt: 'ContextInfo.get_full_tick',
      },
      order_target_value: {
        joinquant: 'order_target_value',
        ptrade: 'order_target_percent',
        qmt: 'order_target_volume',
      },
      ...
      get_index_stocks: {
        joinquant: 'get_index_stocks',
        ptrade: null,
        qmt: 'ContextInfo.get_stock_list_in_sector',
      },
    };
  ...
  private generateSuggestions(
    issues: CompatibilityScore['issues'],
