...
export class StrategyAnalyzer {
  ...
  analyze(code: string, fileName?: string): StrategyAnalysis {
    ...
    logger.info(
      `策略分析完成: ${analysis.factors.length}个因子, ${analysis.issues.length}个问题`,
      MODULE
    );
    return analysis;
  }
  ...
  private extractRiskControl(code: string): RiskControlInfo {
    ...
    // 提取止损
    const stopLossMatch =
      code.match(/stop_loss\s*[=:]\s*([\d.]+)/i) || code.match(/止损.*?([\d.]+)/);
    if (stopLossMatch) {
    ...
    const takeProfitMatch =
      code.match(/take_profit\s*[=:]\s*([\d.]+)/i) || code.match(/止盈.*?([\d.]+)/);
    if (takeProfitMatch) {
    ...
    // 提取仓位
    const maxPosMatch =
      code.match(/max_position\s*[=:]\s*([\d.]+)/i) || code.match(/最大仓位.*?([\d.]+)/);
    if (maxPosMatch) {
  ...
  private extractStockSelection(code: string): StockSelectionInfo {
    ...
    // 选股数量
    const topNMatch = code.match(/g\.stock_num\s*=\s*(\d+)/i) || code.match(/top.*?(\d+)/i);
    if (topNMatch) {
