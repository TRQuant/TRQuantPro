...
export class ManualLearner {
  ...
  async learnFromManual(): Promise<{
    ...
    this.store.save();
    this.log(
      `学习完成！扫描${this.stats.filesScanned}个文件，提取${this.stats.patternsExtracted}个模式，${this.stats.practicesExtracted}个最佳实践`
    );
  ...
  private addPatternFromCode(
    ...
    if (funcName.includes('select') || funcName.includes('filter') || funcName.includes('stock')) {
      ...
      funcName.includes('stop') ||
      funcName.includes('risk') ||
      funcName.includes('position')
    ) {
      category = 'risk';
    } else if (
      funcName.includes('timing') ||
      funcName.includes('signal') ||
      funcName.includes('trend')
    ) {
      category = 'timing';
    }
  ...
  getChapterKnowledge(
    ...
    const patterns = this.store.getPatterns().filter((p) => p.description.includes(sourcePrefix));

    const practices = this.store
      .getBestPractices()
      .filter((bp) => bp.references.some((r) => r.includes(sourcePrefix)));
