...
import {
  registerDeveloperProvider,
  localBackup,
  gitCommit,
  refreshSystem,
} from './providers/developerProvider';
      ...
      async () => {
        ...
        try {
          ...
          const projectName = await vscode.window.showInputBox({
            ...
            placeHolder: '输入项目名称（如：睿智金龙、飘逸紫竹、灵动碧鹤）',
            ignoreFocusOut: true,
            validateInput: (value) => {
          ...
          if (fs.existsSync(templatePath)) {
            ...
            if (hotMainlines.length > 0) {
              const mainlinesStr = hotMainlines
                .map((m) => `    ('${m.name}', ${JSON.stringify(m.industries)}),`)
                .join('\n');
              strategyContent = strategyContent.replace(
                /HOT_MAINLINES = \[\n {4}\('人工智能'.*?\n {4}\('半导体'.*?\n\]/s,
                `HOT_MAINLINES = [\n${mainlinesStr}\n]`
            ...
            strategyContent = strategyContent.replace(/\{\{/g, '{').replace(/\}\}/g, '}');
          } else {
            // 回退到简化模板
...
`;
          ...
          vscode.window
            .showInformationMessage(`✅ 项目 "${finalName}" 创建成功！`, '运行回测')
            .then((selection) => {
              if (selection === '运行回测') {
    ...
    // 注册工作流步骤面板（各步骤的详细面板）
    registerWorkflowStepPanels(context, client);
      ...
      vscode.commands.registerCommand('trquant.openAShareManual', async () => {
        try {
          ...
          if (!fs.existsSync(manualPath)) {
            ...
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            if (workspaceFolder) {
              const workspaceManualPath = path.join(
          ...
          try {
            ...
            setTimeout(async () => {
              ...
              try {
                ...
                vscode.window.showWarningMessage(
                  '无法自动打开浏览器，请手动访问 http://localhost:4321'
                );
              }
            }, 2000); // 参考桌面软件：等待2秒
          } catch (spawnError) {
            ...
            vscode.window.showErrorMessage(
              `启动失败: ${spawnError instanceof Error ? spawnError.message : String(spawnError)}`
            );
          }
        } catch (err) {
    ...
    try {
      ...
      // 备用方案：显示通知让用户手动打开
      vscode.window.showErrorMessage(`主控制台打开失败: ${err}`, '重试').then((selection) => {
        if (selection === '重试') {
...
function registerCommands(context: vscode.ExtensionContext): void {
  const commands: Array<{ id: string; handler: () => Promise<void> }> = [
    ...
    {
      id: 'trquant.localBackup',
      handler: async () => {
        console.log('[TRQuant] 本地备份');
        logger.info('本地备份', MODULE);
        await localBackup(context);
      },
    },
    {
      id: 'trquant.gitCommit',
      handler: async () => {
        console.log('[TRQuant] Git Commit');
        logger.info('Git Commit', MODULE);
