...
<h2 id="section-11-3-2">📈 11.3.2 步骤2：市场趋势</h2>

市场趋势分析识别市场短/中/长期趋势，判断市场状态。
...
- **趋势图表**：显示短/中/长期趋势图表
- **指数趋势表**：显示主要指数的趋势分析
- **风格轮动图**：显示价值、成长、动量等风格的评分

### 注意事项

- 市场分析需要数据支持，确保数据源正常
- 分析结果会保存到MongoDB，便于历史对比
- 市场状态会影响后续的因子推荐和策略生成
...
<h2 id="section-11-3-4">📦 11.3.4 步骤4：候选池构建</h2>

候选池构建基于投资主线和筛选条件，构建候选股票池。
...
<h2 id="section-11-3-5">📊 11.3.5 步骤5：因子构建</h2>

因子构建包括因子推荐、因子计算、因子组合等。
...
- **因子推荐列表**：显示推荐的因子及其权重
- **因子计算结果**：显示因子值和统计信息
- **因子表现图表**：显示因子的IC、IR等指标

### 注意事项

- 因子推荐基于市场状态，不同市场状态推荐不同因子
- 因子计算需要数据支持，确保数据完整
- 因子组合建议使用3-5个因子，避免过度拟合

<h2 id="section-11-3-6">🛠️ 11.3.6 步骤6：策略生成</h2>
