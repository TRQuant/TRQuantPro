---
title: "4.2 主线筛选"
description: "深入解析主线筛选机制，包括评分筛选、行业筛选、时间筛选和综合筛选算法"
lang: "zh-CN"
layout: "/src/layouts/HandbookLayout.astro"
currentBook: "ashare-book6"
updateDate: "2025-12-12"
---

# 🔍 4.2 主线筛选

> **核心摘要：**
> 
> 本节系统介绍TRQuant系统的主线筛选机制，包括评分筛选、行业筛选、时间筛选和综合筛选算法。通过理解最低评分阈值、评分排名、评分区间筛选方法，行业分类、行业权重、行业轮动筛选逻辑，时间窗口、时间持续性、时间趋势筛选机制，以及多条件组合筛选和智能筛选规则，帮助开发者掌握主线筛选的核心实现，为构建高效的主线筛选系统奠定基础。

## 📋 章节概览

<script>
function scrollToSection(sectionId) {
  const element = document.getElementById(sectionId);
  if (element) {
    const headerOffset = 100;
    const elementPosition = element.getBoundingClientRect().top;
    const offsetPosition = elementPosition + window.pageYOffset - headerOffset;
    window.scrollTo({
      top: offsetPosition,
      behavior: 'smooth'
    });
  }
}
</script>

<div class="section-overview">
  <div class="section-item" onclick="scrollToSection('section-4-2-1')">
    <h4>📊 4.2.1 评分筛选</h4>
    <p>最低评分阈值、评分排名、评分区间筛选</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-4-2-2')">
    <h4>🏭 4.2.2 行业筛选</h4>
    <p>行业分类、行业权重、行业轮动筛选</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-4-2-3')">
    <h4>⏰ 4.2.3 时间筛选</h4>
    <p>时间窗口、时间持续性、时间趋势筛选</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-4-2-4')">
    <h4>🔀 4.2.4 综合筛选</h4>
    <p>多条件组合筛选、智能筛选规则、筛选结果排序</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-4-2-5')">
    <h4>🔄 4.2.5 自动化实现</h4>
    <p>自动筛选更新、自动筛选规则调整、自动筛选报告</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-4-2-6')">
    <h4>🛠️ 4.2.6 MCP工具使用</h4>
    <p>使用MCP工具查询筛选相关文档、收集研究资料</p>
  </div>
</div>

## 🎯 学习目标

通过本节学习，您将能够：

- **掌握评分筛选**：理解最低评分阈值、评分排名、评分区间筛选的实现方法
- **理解行业筛选**：掌握行业分类、行业权重、行业轮动筛选的逻辑和算法
- **熟悉时间筛选**：理解时间窗口、时间持续性、时间趋势筛选的机制
- **了解综合筛选**：掌握多条件组合筛选、智能筛选规则和筛选结果排序
- **实现自动化**：理解自动筛选更新、自动筛选规则调整和自动筛选报告
- **使用MCP工具**：掌握使用MCP工具进行筛选相关研究

<h2 id="section-4-2-1">📊 4.2.1 评分筛选</h2>

评分筛选根据主线的综合评分进行筛选，确保筛选出的主线具有足够的投资价值。

### 设计原则

<div class="key-points">
  <div class="key-point">
    <h4>🎯 阈值可配置</h4>
    <p>最低评分阈值可配置，适应不同投资策略</p>
  </div>
  <div class="key-point">
    <h4>📊 排名筛选</h4>
    <p>支持选择排名前N的主线，控制主线数量</p>
  </div>
  <div class="key-point">
    <h4>📈 区间筛选</h4>
    <p>支持选择评分在指定区间的主线</p>
  </div>
  <div class="key-point">
    <h4>🔄 动态调整</h4>
    <p>根据市场环境动态调整筛选阈值</p>
  </div>
</div>

### 最低评分阈值筛选

设置最低评分阈值，筛选出评分高于阈值的主线：

```python
from typing import List
from core.base_mainline import Mainline

def filter_by_min_score(
    mainlines: List[Mainline],
    min_score: float = 60.0
) -> List[Mainline]:
    """
    根据最低评分筛选主线
    
    Args:
        mainlines: 主线列表
        min_score: 最低评分阈值（默认60分）
    
    Returns:
        筛选后的主线列表
    """
    filtered = [
        mainline for mainline in mainlines
        if mainline.score.total_score >= min_score
    ]
    return filtered

# 使用示例
mainlines = get_all_mainlines()
filtered_mainlines = filter_by_min_score(mainlines, min_score=75.0)
print(f"筛选出{len(filtered_mainlines)}条主线（评分≥75分）")
```

### 评分排名筛选

选择排名前N的主线：

```python
def filter_by_top_n(
    mainlines: List[Mainline],
    top_n: int = 10
) -> List[Mainline]:
    """
    根据评分排名筛选主线
    
    Args:
        mainlines: 主线列表
        top_n: 排名前N（默认10）
    
    Returns:
        筛选后的主线列表（按评分降序排列）
    """
    # 按评分排序
    sorted_mainlines = sorted(
        mainlines,
        key=lambda m: m.score.total_score,
        reverse=True
    )
    
    # 取前N条
    return sorted_mainlines[:top_n]

# 使用示例
mainlines = get_all_mainlines()
top_10_mainlines = filter_by_top_n(mainlines, top_n=10)
for i, mainline in enumerate(top_10_mainlines, 1):
    print(f"{i}. {mainline.name}: {mainline.score.total_score:.2f}分")
```

### 评分区间筛选

选择评分在指定区间的主线：

```python
def filter_by_score_range(
    mainlines: List[Mainline],
    min_score: float = None,
    max_score: float = None
) -> List[Mainline]:
    """
    根据评分区间筛选主线
    
    Args:
        mainlines: 主线列表
        min_score: 最低评分（可选）
        max_score: 最高评分（可选）
    
    Returns:
        筛选后的主线列表
    """
    filtered = mainlines
    
    if min_score is not None:
        filtered = [
            m for m in filtered
            if m.score.total_score >= min_score
        ]
    
    if max_score is not None:
        filtered = [
            m for m in filtered
            if m.score.total_score <= max_score
        ]
    
    return filtered

# 使用示例
mainlines = get_all_mainlines()
# 筛选评分在70-90之间的主线
filtered_mainlines = filter_by_score_range(
    mainlines,
    min_score=70.0,
    max_score=90.0
)
```

<h2 id="section-4-2-2">🏭 4.2.2 行业筛选</h2>

行业筛选根据主线相关的行业进行筛选，支持行业分类、行业权重和行业轮动筛选。

### 行业分类筛选

按行业分类筛选主线：

```python
def filter_by_industries(
    mainlines: List[Mainline],
    industries: List[str],
    match_all: bool = False
) -> List[Mainline]:
    """
    根据行业筛选主线
    
    Args:
        mainlines: 主线列表
        industries: 行业列表
        match_all: 是否要求匹配所有行业（默认False，匹配任一即可）
    
    Returns:
        筛选后的主线列表
    """
    # 设计原理：行业筛选支持两种模式
    # 原因：不同场景需要不同的筛选逻辑
    # match_all=True：要求匹配所有行业（交集），适用于精确筛选
    # match_all=False：匹配任一行业即可（并集），适用于宽泛筛选
    # 为什么这样设计：提供灵活性，适应不同的筛选需求
    if match_all:
        # 设计原理：交集筛选（AND逻辑）
        # 原因：要求主线同时包含所有指定行业，筛选更严格
        # 使用场景：需要主线同时涉及多个相关行业时
        filtered = [
            mainline for mainline in mainlines
            if all(ind in mainline.sectors for ind in industries)
        ]
    else:
        # 设计原理：并集筛选（OR逻辑）
        # 原因：主线包含任一指定行业即可，筛选更宽松
        # 使用场景：需要主线涉及任一相关行业时
        filtered = [
            mainline for mainline in mainlines
            if any(ind in mainline.sectors for ind in industries)
        ]
    
    return filtered

# 使用示例
mainlines = get_all_mainlines()
# 筛选包含"新能源"或"人工智能"的主线
filtered_mainlines = filter_by_industries(
    mainlines,
    industries=["新能源", "人工智能"],
    match_all=False
)
```

### 行业权重筛选

根据行业权重筛选主线，优先选择权重较高的行业：

```python
def filter_by_industry_weights(
    mainlines: List[Mainline],
    industry_weights: Dict[str, float],
    min_weight: float = 0.1
) -> List[Mainline]:
    """
    根据行业权重筛选主线
    
    Args:
        mainlines: 主线列表
        industry_weights: 行业权重字典 {行业名: 权重}
        min_weight: 最小权重阈值
    
    Returns:
        筛选后的主线列表（按行业权重排序）
    """
    filtered = []
    
    # 设计原理：行业权重筛选
    # 原因：不同行业的重要性不同，需要根据权重筛选
    # 实现方式：计算主线的行业权重得分，按得分筛选和排序
    # 为什么这样设计：优先选择权重高的行业，提高筛选的针对性
    for mainline in mainlines:
        # 设计原理：累加行业权重得分
        # 原因：主线可能涉及多个行业，需要累加权重
        # 实现方式：遍历主线的所有行业，累加对应的权重
        industry_score = 0.0
        for sector in mainline.sectors:
            weight = industry_weights.get(sector, 0.0)
            industry_score += weight
        
        # 设计原理：阈值筛选
        # 原因：只保留行业权重得分达到阈值的主线
        # 为什么这样设计：过滤掉行业权重过低的主线，提高筛选质量
        if industry_score >= min_weight:
            # 设计原理：临时属性存储得分
            # 原因：便于后续排序，不修改主线原始数据
            mainline.industry_score = industry_score
            filtered.append(mainline)
    
    # 设计原理：按得分排序
    # 原因：优先展示权重高的主线，便于用户选择
    # 实现方式：使用sort按industry_score降序排序
    filtered.sort(key=lambda m: m.industry_score, reverse=True)
    
    return filtered

# 使用示例
mainlines = get_all_mainlines()
industry_weights = {
    "新能源": 0.3,
    "人工智能": 0.25,
    "半导体": 0.2,
    "医药": 0.15,
    "消费": 0.1
}
filtered_mainlines = filter_by_industry_weights(
    mainlines,
    industry_weights,
    min_weight=0.2
)
```

### 行业轮动筛选

考虑行业轮动因素，筛选处于轮动上升期的行业：

```python
def filter_by_industry_rotation(
    mainlines: List[Mainline],
    rotation_signal: Dict[str, str]
) -> List[Mainline]:
    """
    根据行业轮动信号筛选主线
    
    Args:
        mainlines: 主线列表
        rotation_signal: 行业轮动信号字典 {行业名: "up"/"down"/"neutral"}
    
    Returns:
        筛选后的主线列表（优先选择轮动上升的行业）
    """
    filtered = []
    
    for mainline in mainlines:
        # 检查主线的行业轮动信号
        rotation_scores = []
        for sector in mainline.sectors:
            signal = rotation_signal.get(sector, "neutral")
            if signal == "up":
                rotation_scores.append(1.0)
            elif signal == "neutral":
                rotation_scores.append(0.5)
            else:  # down
                rotation_scores.append(0.0)
        
        # 如果平均轮动得分>0.5，则保留
        avg_rotation_score = sum(rotation_scores) / len(rotation_scores) if rotation_scores else 0.0
        if avg_rotation_score > 0.5:
            mainline.rotation_score = avg_rotation_score
            filtered.append(mainline)
    
    # 按轮动得分排序
    filtered.sort(key=lambda m: m.rotation_score, reverse=True)
    
    return filtered

# 使用示例
mainlines = get_all_mainlines()
rotation_signal = {
    "新能源": "up",
    "人工智能": "up",
    "半导体": "neutral",
    "医药": "down",
    "消费": "neutral"
}
filtered_mainlines = filter_by_industry_rotation(mainlines, rotation_signal)
```

<h2 id="section-4-2-3">⏰ 4.2.3 时间筛选</h2>

时间筛选根据主线的时间特征进行筛选，包括时间窗口、时间持续性和时间趋势。

### 时间窗口筛选

选择特定时间窗口内的主线：

```python
from datetime import datetime, timedelta

def filter_by_time_window(
    mainlines: List[Mainline],
    days: int = 30
) -> List[Mainline]:
    """
    根据时间窗口筛选主线
    
    Args:
        mainlines: 主线列表
        days: 时间窗口（天数，默认30天）
    
    Returns:
        筛选后的主线列表（在时间窗口内启动或更新的主线）
    """
    cutoff_date = datetime.now() - timedelta(days=days)
    
    filtered = []
    for mainline in mainlines:
        # 检查主线启动日期或更新日期
        start_date = datetime.strptime(mainline.start_date, "%Y-%m-%d") if mainline.start_date else None
        updated_date = datetime.strptime(mainline.updated_at, "%Y-%m-%d") if mainline.updated_at else None
        
        # 如果启动日期或更新日期在时间窗口内，则保留
        if (start_date and start_date >= cutoff_date) or \
           (updated_date and updated_date >= cutoff_date):
            filtered.append(mainline)
    
    return filtered

# 使用示例
mainlines = get_all_mainlines()
# 筛选最近30天内启动或更新的主线
filtered_mainlines = filter_by_time_window(mainlines, days=30)
```

### 时间持续性筛选

选择持续性较好的主线：

```python
def filter_by_persistence(
    mainlines: List[Mainline],
    min_days: int = 5
) -> List[Mainline]:
    """
    根据时间持续性筛选主线
    
    Args:
        mainlines: 主线列表
        min_days: 最小持续天数（默认5天）
    
    Returns:
        筛选后的主线列表（持续天数≥min_days的主线）
    """
    filtered = []
    
    for mainline in mainlines:
        if mainline.start_date:
            start_date = datetime.strptime(mainline.start_date, "%Y-%m-%d")
            duration = (datetime.now() - start_date).days
            
            if duration >= min_days:
                mainline.duration = duration
                filtered.append(mainline)
    
    # 按持续天数排序
    filtered.sort(key=lambda m: m.duration, reverse=True)
    
    return filtered

# 使用示例
mainlines = get_all_mainlines()
# 筛选持续至少5天的主线
filtered_mainlines = filter_by_persistence(mainlines, min_days=5)
```

### 时间趋势筛选

选择趋势向上的主线（评分持续上升）：

```python
def filter_by_trend(
    mainlines: List[Mainline],
    min_trend_days: int = 3
) -> List[Mainline]:
    """
    根据时间趋势筛选主线
    
    Args:
        mainlines: 主线列表
        min_trend_days: 最小趋势天数（默认3天）
    
    Returns:
        筛选后的主线列表（评分持续上升的主线）
    """
    filtered = []
    
    for mainline in mainlines:
        # 获取历史评分数据（需要从数据库或缓存获取）
        historical_scores = get_historical_scores(mainline.id, days=min_trend_days)
        
        if len(historical_scores) >= min_trend_days:
            # 检查评分是否持续上升
            is_rising = all(
                historical_scores[i] < historical_scores[i+1]
                for i in range(len(historical_scores) - 1)
            )
            
            if is_rising:
                mainline.trend_score = historical_scores[-1] - historical_scores[0]
                filtered.append(mainline)
    
    # 按趋势得分排序
    filtered.sort(key=lambda m: m.trend_score, reverse=True)
    
    return filtered

# 使用示例
mainlines = get_all_mainlines()
# 筛选评分持续上升的主线
filtered_mainlines = filter_by_trend(mainlines, min_trend_days=3)
```

<h2 id="section-4-2-4">🔀 4.2.4 综合筛选</h2>

综合筛选支持多条件组合筛选，提供灵活的筛选能力。

### 多条件组合筛选

```python
@dataclass
class FilterCriteria:
    """筛选条件"""
    min_score: float = None              # 最低评分
    max_score: float = None              # 最高评分
    top_n: int = None                    # 排名前N
    industries: List[str] = None         # 行业列表
    match_all_industries: bool = False   # 是否匹配所有行业
    industry_weights: Dict[str, float] = None  # 行业权重
    min_industry_weight: float = None     # 最小行业权重
    time_window_days: int = None         # 时间窗口（天数）
    min_persistence_days: int = None     # 最小持续天数
    require_trend_up: bool = False       # 是否要求趋势向上
    min_trend_days: int = None           # 最小趋势天数

class MainlineFilter:
    """主线筛选器"""
    
    def __init__(self):
        self.criteria = FilterCriteria()
    
    def filter(
        self,
        mainlines: List[Mainline],
        criteria: FilterCriteria = None
    ) -> List[Mainline]:
        """
        综合筛选主线
        
        Args:
            mainlines: 主线列表
            criteria: 筛选条件
        
        Returns:
            筛选后的主线列表
        """
        if criteria is None:
            criteria = self.criteria
        
        filtered = mainlines
        
        # 1. 评分筛选
        if criteria.min_score is not None or criteria.max_score is not None:
            filtered = filter_by_score_range(
                filtered,
                min_score=criteria.min_score,
                max_score=criteria.max_score
            )
        
        # 2. 排名筛选
        if criteria.top_n is not None:
            filtered = filter_by_top_n(filtered, top_n=criteria.top_n)
        
        # 3. 行业筛选
        if criteria.industries:
            filtered = filter_by_industries(
                filtered,
                industries=criteria.industries,
                match_all=criteria.match_all_industries
            )
        
        # 4. 行业权重筛选
        if criteria.industry_weights and criteria.min_industry_weight:
            filtered = filter_by_industry_weights(
                filtered,
                industry_weights=criteria.industry_weights,
                min_weight=criteria.min_industry_weight
            )
        
        # 5. 时间窗口筛选
        if criteria.time_window_days:
            filtered = filter_by_time_window(
                filtered,
                days=criteria.time_window_days
            )
        
        # 6. 持续性筛选
        if criteria.min_persistence_days:
            filtered = filter_by_persistence(
                filtered,
                min_days=criteria.min_persistence_days
            )
        
        # 7. 趋势筛选
        if criteria.require_trend_up and criteria.min_trend_days:
            filtered = filter_by_trend(
                filtered,
                min_trend_days=criteria.min_trend_days
            )
        
        return filtered
    
    def sort(
        self,
        mainlines: List[Mainline],
        sort_by: str = "score",
        order: str = "desc"
    ) -> List[Mainline]:
        """
        排序筛选结果
        
        Args:
            mainlines: 主线列表
            sort_by: 排序字段（score/duration/industry_score/rotation_score）
            order: 排序顺序（asc/desc）
        
        Returns:
            排序后的主线列表
        """
        reverse = (order == "desc")
        
        if sort_by == "score":
            sorted_mainlines = sorted(
                mainlines,
                key=lambda m: m.score.total_score,
                reverse=reverse
            )
        elif sort_by == "duration":
            sorted_mainlines = sorted(
                mainlines,
                key=lambda m: getattr(m, 'duration', 0),
                reverse=reverse
            )
        elif sort_by == "industry_score":
            sorted_mainlines = sorted(
                mainlines,
                key=lambda m: getattr(m, 'industry_score', 0),
                reverse=reverse
            )
        elif sort_by == "rotation_score":
            sorted_mainlines = sorted(
                mainlines,
                key=lambda m: getattr(m, 'rotation_score', 0),
                reverse=reverse
            )
        else:
            sorted_mainlines = mainlines
        
        return sorted_mainlines

# 使用示例
filter = MainlineFilter()

# 设置筛选条件
criteria = FilterCriteria(
    min_score=70.0,              # 最低评分70分
    top_n=10,                    # 排名前10
    industries=["新能源", "人工智能"],  # 行业筛选
    time_window_days=30,         # 最近30天
    min_persistence_days=5,      # 持续至少5天
    require_trend_up=True,       # 要求趋势向上
    min_trend_days=3            # 趋势至少3天
)

# 执行筛选
mainlines = get_all_mainlines()
filtered_mainlines = filter.filter(mainlines, criteria)

# 排序
sorted_mainlines = filter.sort(
    filtered_mainlines,
    sort_by="score",
    order="desc"
)

# 生成筛选报告
report = filter.generate_report(sorted_mainlines, criteria)
```

<h2 id="section-4-2-5">🔄 4.2.5 自动化实现</h2>

主线筛选模块支持自动化运行，定时筛选主线，自动更新结果。

```python
import schedule
import time
from datetime import datetime

class MainlineFilterMonitor:
    """主线筛选监控器"""
    
    def __init__(self):
        self.filter = MainlineFilter()
        self.current_filtered = []
        self.last_update_time = None
    
    def auto_filter(self):
        """自动筛选主线"""
        # 获取所有主线
        mainlines = get_all_mainlines()
        
        # 设置筛选条件（可根据市场环境动态调整）
        criteria = self._get_auto_criteria()
        
        # 执行筛选
        filtered = self.filter.filter(mainlines, criteria)
        
        # 排序
        sorted_mainlines = self.filter.sort(
            filtered,
            sort_by="score",
            order="desc"
        )
        
        # 更新结果
        self.current_filtered = sorted_mainlines
        self.last_update_time = datetime.now()
        
        logger.info(f"自动筛选完成，筛选出{len(sorted_mainlines)}条主线")
        
        return sorted_mainlines
    
    def _get_auto_criteria(self) -> FilterCriteria:
        """获取自动筛选条件（可根据市场环境动态调整）"""
        # 获取市场状态
        market_status = get_market_status()
        
        if market_status['regime'] == 'risk_on':
            # 牛市：提高评分阈值，关注技术形态
            return FilterCriteria(
                min_score=75.0,
                top_n=10,
                require_trend_up=True
            )
        elif market_status['regime'] == 'risk_off':
            # 熊市：降低评分阈值，关注估值
            return FilterCriteria(
                min_score=60.0,
                top_n=5
            )
        else:
            # 震荡市：中等阈值
            return FilterCriteria(
                min_score=70.0,
                top_n=8
            )
    
    def start_auto_filter(self, interval_minutes: int = 60):
        """
        启动自动筛选
        
        Args:
            interval_minutes: 筛选间隔（分钟）
        """
        schedule.every(interval_minutes).minutes.do(self.auto_filter)
        
        # 立即执行一次
        self.auto_filter()
        
        # 持续运行
        while True:
            schedule.run_pending()
            time.sleep(60)
```

<h2 id="section-4-2-6">🛠️ 4.2.6 MCP工具使用</h2>

主线筛选模块与MCP工具集成，支持知识库查询、数据收集等功能。

### KB MCP Server工具

#### kb.query

查询知识库，获取主线筛选相关的文档和代码：

```python
# 查询主线筛选相关的知识
results = mcp_client.call_tool(
    "kb.query",
    {
        "query": "主线筛选 评分筛选 行业筛选 时间筛选",
        "collection": "manual_kb",
        "top_k": 5
    }
)
```

### Data Collector MCP工具

#### data_collector.crawl_web

爬取网页内容，收集主线筛选相关的研究资料：

```python
# 爬取筛选算法相关网页
content = mcp_client.call_tool(
    "data_collector.crawl_web",
    {
        "url": "https://example.com/filtering-algorithm",
        "extract_text": True
    }
)
```

## 🔗 相关章节

- **第2章：数据源模块** - 了解数据获取机制，为主线筛选提供数据支撑
- **第3章：市场分析模块** - 市场分析结果用于筛选条件调整
- **第4章：投资主线识别** - 了解主线识别模块的整体设计
- **第4.1节：主线评分** - 主线评分结果用于筛选
- **第5章：候选池构建** - 筛选后的主线用于候选池构建
- **第6章：因子库** - 筛选后的主线用于因子推荐
- **第10章：开发指南** - 了解主线筛选模块的开发规范

## 🔮 总结与展望

<div class="summary-outlook">
  <h3>本节回顾</h3>
  <p>本节系统介绍了主线筛选功能，包括评分筛选、行业筛选、时间筛选的算法和实现。通过理解主线筛选的核心技术，帮助开发者掌握如何从众多主线中筛选出最有价值的投资主线，为候选池构建提供方向指引。</p>
  
  <h3>下节预告</h3>
  <p>掌握了主线筛选后，下一节将介绍主线识别引擎，包括三层分析框架、主线发现流程和生命周期管理。通过理解主线识别引擎的设计原理，帮助开发者掌握如何构建智能化的投资主线识别系统。</p>
  
  <a href="/ashare-book6/004_Chapter4_Mainline_Identification/4.3_Mainline_Engine_CN" class="next-section">
    继续学习：4.3 主线识别引擎 →
  </a>
</div>

> **适用版本**: v1.0.0+  
> **最后更新**: 2025-12-12
