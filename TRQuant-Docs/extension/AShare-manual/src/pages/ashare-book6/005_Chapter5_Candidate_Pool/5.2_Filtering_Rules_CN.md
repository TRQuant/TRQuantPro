---
title: "5.2 筛选规则"
description: "深入解析筛选规则机制，包括基础筛选、主线筛选、行业筛选、技术筛选和规则组合"
lang: "zh-CN"
layout: "/src/layouts/HandbookLayout.astro"
currentBook: "ashare-book6"
updateDate: "2025-12-12"
---

# 🔍 5.2 筛选规则

> **核心摘要：**
> 
> 本节系统介绍TRQuant系统的筛选规则机制，包括基础筛选规则（ST、停牌、涨跌停过滤）、主线筛选规则（基于投资主线的行业匹配）、行业筛选规则（行业分类、行业权重）、技术筛选规则（技术突破、价格趋势、成交量配合）和规则组合（逻辑组合、优先级设置、规则权重）。通过理解各类筛选规则的配置、执行逻辑和组合方法，帮助开发者掌握筛选规则的核心实现，为构建灵活高效的股票筛选系统奠定基础。

## 📋 章节概览

<script>
function scrollToSection(sectionId) {
  const element = document.getElementById(sectionId);
  if (element) {
    const headerOffset = 100;
    const elementPosition = element.getBoundingClientRect().top;
    const offsetPosition = elementPosition + window.pageYOffset - headerOffset;
    window.scrollTo({
      top: offsetPosition,
      behavior: 'smooth'
    });
  }
}
</script>

<div class="section-overview">
  <div class="section-item" onclick="scrollToSection('section-5-2-1')">
    <h4>🛡️ 5.2.1 基础筛选规则</h4>
    <p>ST、停牌、涨跌停过滤、流动性筛选</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-5-2-2')">
    <h4>🔥 5.2.2 主线筛选规则</h4>
    <p>基于投资主线的行业匹配、龙头股筛选</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-5-2-3')">
    <h4>📊 5.2.3 技术筛选规则</h4>
    <p>技术突破、价格趋势、成交量配合、均线突破</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-5-2-4')">
    <h4>💼 5.2.4 基本面筛选规则</h4>
    <p>ROE、净利润增长、营收增长、财务指标筛选</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-5-2-5')">
    <h4>🔀 5.2.5 规则组合与执行</h4>
    <p>逻辑组合、优先级设置、规则权重、规则执行引擎</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-5-2-6')">
    <h4>🛠️ 5.2.6 MCP工具使用</h4>
    <p>使用MCP工具查询筛选规则相关文档</p>
  </div>
</div>

## 🎯 学习目标

通过本节学习，您将能够：

- **掌握基础筛选**：理解ST、停牌、涨跌停过滤、流动性筛选的实现方法
- **理解主线筛选**：掌握基于投资主线的行业匹配和龙头股筛选逻辑
- **熟悉技术筛选**：理解技术突破、价格趋势、成交量配合、均线突破的筛选条件
- **了解基本面筛选**：掌握ROE、净利润增长、营收增长等财务指标筛选
- **实现规则组合**：理解逻辑组合、优先级设置、规则权重和规则执行引擎
- **使用MCP工具**：掌握使用MCP工具进行筛选规则相关研究

<h2 id="section-5-2-1">🛡️ 5.2.1 基础筛选规则</h2>

基础筛选规则用于过滤不可交易的股票，确保候选池中的股票都是可交易的。

### 设计原则

<div class="key-points">
  <div class="key-point">
    <h4>🛡️ 安全性优先</h4>
    <p>优先过滤ST、停牌、涨跌停等不可交易股票</p>
  </div>
  <div class="key-point">
    <h4>💧 流动性保障</h4>
    <p>确保股票有足够的流动性，避免无法成交</p>
  </div>
  <div class="key-point">
    <h4>⚡ 高效执行</h4>
    <p>基础筛选规则执行速度快，优先执行</p>
  </div>
  <div class="key-point">
    <h4>🔧 可配置</h4>
    <p>筛选条件可配置，适应不同市场环境</p>
  </div>
</div>

### ST股票过滤

过滤ST、*ST等特殊处理股票：

<CodeFromFile 
  filePath="code_library/005_Chapter5_Candidate_Pool/5.2/code_5_2_filter_st_stocks.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
def filter_st_stocks(stocks: List[str], date: str = None) -> List[str]:
    """
    过滤ST股票
    
    Args:
        stocks: 股票代码列表
        date: 日期
    
    Returns:
        过滤后的股票代码列表
    """
    filtered = []
    
    for stock_code in stocks:
        # 检查是否为ST股票
        stock_info = get_stock_info(stock_code, date=date)
        
        # ST股票通常名称包含"ST"或"*ST"
        if stock_info and ("ST" in stock_info.get("name", "") or 
                          "*ST" in stock_info.get("name", "")):
            continue
        
        # 检查是否被ST处理
<CodeFromFile 
  filePath="code_library/005_Chapter5_Candidate_Pool/5.2/code_5_2_filter_suspended_stocks.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
def filter_suspended_stocks(stocks: List[str], date: str = None) -> List[str]:
    """
    过滤停牌股票
    
    Args:
        stocks: 股票代码列表
        date: 日期
    
    Returns:
        过滤后的股票代码列表
    """
    filtered = []
    
    for stock_code in stocks:
        # 检查是否停牌
        is_suspended<CodeFromFile 
  filePath="code_library/005_Chapter5_Candidate_Pool/5.2/code_5_2_filter_limit_stocks.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
def filter_limit_stocks(
    stocks: List[str], 
    date: str = None,
    filter_limit_up: bool = True,
    filter_limit_down: bool = True
) -> List[str]:
    """
    过滤涨跌停股票
    
    Args:
        stocks: 股票代码列表
        date: 日期
        filter_limit_up: 是否过滤涨停股票
        filter_limit_down: 是否过滤跌停股票
    
    Returns:
        过滤后的股票代码列表
    """
    filtered = []
    
    for stock_code in stocks:
        # 获取涨跌幅
        price_data = get_price_data(stock_code, date=date)
        if price_data.empty:
            continue
        
        change_pct = price_data['change_pct'].iloc[-1]
        
        # 涨停（涨跌幅 >= 9.5%）
        is_limit_up = change_pct >= 9.5
        
        # 跌停（涨跌幅 <= -9.5%）
        is_limit_down = change_pct <= -9.5
        
        # 根据策略决定是否过滤
        if filter_limit_up and is_limit_up:
            <CodeFromFile 
  filePath="code_library/005_Chapter5_Candidate_Pool/5.2/code_5_2_filter_by_liquidity.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
def filter_by_liquidity(
    stocks: List[str],
    date: str = None,
    min_turnover: float = 0.01,  # 最小换手率
    min_volume: float = 1000000   # 最小成交量（手）
) -> List[str]:
    """
    根据流动性筛选股票
    
    Args:
        stocks: 股票代码列表
        date: 日期
        min_turnover: 最小换手率
        min_volume: 最小成交量（手）
    
    Returns:
        过滤后的股票代码列表
    """
    filtered = []
    
    for stock_code in stocks:
        # 获取成交量和换手率
        market_data = get_market_data(stock_code, date=date)
        if market_data.empty:
            continue
        
        volume = market_data['volume'].iloc[-1]
        turnover = market_data.get('turno<CodeFromFile 
  filePath="code_library/005_Chapter5_Candidate_Pool/5.2/code_5_2___init__.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
class BasicFilter:
    """基础筛选器"""
    
    def __init__(self):
        self.filter_st = True
        self.filter_suspended = True
        self.filter_limit_up = False  # 默认不过滤涨停（可能是有机会的股票）
        self.filter_limit_down = True  # 过滤跌停
        self.min_turnover = 0.01
        self.min_volume = 1000000
    
    def filter(self, stocks: List[str], date: str = None) -> List[str]:
        """
        执行基础筛选
        
        Args:
            stocks: 股票代码列表
            date: 日期
        
        Returns:
            过滤后的股票代码列表
        """
        filtered = stocks
        
        # 1. 过滤ST股票
        if self.filter_st:
            filtered = filter_st_stocks(filtered, date=date)
        
        # 2. 过滤停牌股票
        if self.filter_suspended:
            filtered = filter_suspended_stocks(filtered, date=date)
        
        # 3. 过滤涨跌停股票
        filtered = filter_limit_stocks(
            filtered,
            date=date,
            filter_limit_up=self.filter_limit_up,
            filter_limit_down=self.filter_limit_down
        )
        
        # 4. 流动性筛选
        filtered = filter_by_liquidity(
            filtered,
            date=date,
            min_turnover=self.min_turnover,
            min_volume=s<CodeFromFile 
  filePath="code_library/005_Chapter5_Candidate_Pool/5.2/code_5_2_filter_by_mainline_industry.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
def filter_by_mainline_industry(
    stocks: List[str],
    mainline: Dict,
    date: str = None
) -> List[str]:
    """
    根据主线行业筛选股票
    
    Args:
        stocks: 股票代码列表
        mainline: 主线信息（包含industries字段）
        date: 日期
    
    Returns:
        过滤后的股票代码列表
    """
    # 获取主线相关行业
    mainline_industries = mainline.get("industries", [])
    
    if not mainline_industries:
        return stocks
    
    filtered = []
    
    for stock_code in stocks:
        # 获取股票行业
        stock_info = get_stock_info(stock_code, date=date)
        stock_industry = stock_info.get("indus<CodeFromFile 
  filePath="code_library/005_Chapter5_Candidate_Pool/5.2/code_5_2_filter_mainline_leaders.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
def filter_mainline_leaders(
    stocks: List[str],
    mainline: Dict,
    date: str = None,
    top_n: int = 10
) -> List[str]:
    """
    筛选主线龙头股
    
    Args:
        stocks: 股票代码列表
        mainline: 主线信息（包含stocks字段）
        date: 日期
        top_n: 返回前N只龙头股
    
    Returns:
        龙头股代码列表
    """
    # 获取主线龙头股列表
    mainline_stocks = mainline.get("stocks", [])
    
    if not mainline_stocks:
        return []
    
    # 筛选出在候选池中的龙头股
    leaders = [s for s in stocks if s in mainline_stocks]
    
    # 按市值或评分排序，取前N只
    leaders_with_score = []
    for stock_code in leaders:
        stock_info = get_stock_info(stock_code, date=date)
        market_cap = stock_info.get("market_cap", 0)
        leaders_with_score.append((stock_code, market_cap))
    
    leaders_with_score.sort(key=lambda x: x[1<CodeFromFile 
  filePath="code_library/005_Chapter5_Candidate_Pool/5.2/code_5_2_filter_technical_breakthrough.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
def filter_technical_breakthrough(
    stocks: List[str],
    date: str = None
) -> List[Dict]:
    """
    技术突破筛选
    
    筛选条件：
    - 涨停
    - 放量突破（成交量放大50%以上）
    - 站上均线（如60日均线）
    - 连续上涨（3天以上）
    
    Args:
        stocks: 股票代码列表
        date: 日期
    
    Returns:
        候选股票列表（包含技术信号）
    """
    candidates = []
    
    for stock_code in stocks:
        try:
            # 获取价格数据（向前推120天）
            end_dt = datetime.strptime(date, "%Y-%m-%d") if date else datetime.now()
            start_dt = end_dt - timedelta(days=120)
            start_date = start_dt.strftime("%Y-%m-%d")
            end_date = end_dt.strftime("%Y-%m-%d")
            
            price_data = get_price_data(stock_code, start_date=start_date, end_date=end_date)
            
            if price_data.empty or len(price_data) < 20:
                continue
            
            # 计算技术指标
            latest = price_data.iloc[-1]
            prev = price_data.iloc[-2] if len(price_data) > 1 else latest
            
            # 涨跌幅
            change_pct = ((latest["close"] - prev["close"]) / prev["close"]) * 100 \
                        if prev["close"] > 0 else 0
            
            # 是否涨停（涨跌幅 >= 9.5%）
            is_limit_up = change_pct >= 9.5
            
            # 是否放量（成交量较前一日放大50%以上）
            volume_ratio = latest["volume"] / prev["volume"] if prev["volume"] > 0 else 1
            is_volume_breakout = volume_ratio >= 1.5
            
            # 是否站上均线
            data_len = len(price_data)
            if data_len >= 60:
                ma60 = price_data["close"].tail(60).mean()
                is_ma_breakthrough = latest["close"] > ma60
            elif data_len >= 30:
                ma30 = price_data["close"].tail(30).mean()
                is_ma_breakthrough = latest["close"] > ma30
            else:
                ma_avg = price_data["close"].mean()
                is_ma_breakthrough = latest["close"] > ma_avg
            
            # 连续上涨天数
            consecutive_up = 0
            for i in range(len(price_data) - 1, 0, -1):
                if price_data.iloc[i]["close"] > price_data.iloc[i - 1]["close"]:
                    consecutive_up += 1
                else:
                    break
            
            # 技术面筛选：至少满足一个条件
            if is_limit_up or is_volume_breakout or is_ma_breakthrough or consecutive_up >= 3:
                candidate = {
                    "code": stock_code,
                    "is_limit_up": is_limit_up,
                    "is_volume_breakout": is_volume_breakout,
                    "is_ma_breakthrough": is_ma_breakthrough,
                    "consecutive_up_days": consecutive_up,
                    "change_pct": round(change_pct, 2)
                }
                candidates.append(candidate)
        
        except Exception as e:
         <CodeFromFile 
  filePath="code_library/005_Chapter5_Candidate_Pool/5.2/code_5_2_filter_fundamental.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
def filter_fundamental(
    stocks: List[str],
    date: str = None,
    min_roe: float = 0.10,           # 最小ROE
    min_profit_growth: float = 0.30, # 最小净利润增长率
    min_revenue_growth: float = 0.20 # 最小营收增长率
) -> List[str]:
    """
    财务因子筛选
    
    筛选条件：
    - ROE > 10%
    - 净利润同比增长 > 30%
    - 营收同比增长 > 20%
    
    Args:
        stocks: 股票代码列表
        date: 日期
        min_roe: 最小ROE
        min_profit_growth: 最小净利润增长率
        min_revenue_growth: 最小营收增长率
    
    Returns:
        过滤后的股票代码列表
    """
    filtered = []
    
    # 批量获取财务数据
    batch_size = 1000
    for i in range(0, len(stocks), batch_size):
        batch = stocks[i:i + batch_size]
        
        try:
            # 获取财务指标
            fundamental_data = get_fundamental_data(batch, date=date)
            
            for stock_code in batch:
                if stock_code not in fundamental_data:
                    continue
                
                data = fundamental_data[stock_code]
                
                # 检查ROE
                roe = data.get("roe", 0)
                if roe < min_roe:
                    continue
                
                # 检查净利润增长率
                profit_growth = data.get("profit_growth", 0)
                if profit_growth < min_profit_growth:
                    continue
                
                # 检查营收增长率
                revenue_growth = data.get("revenue_growth", 0)
                if revenue_growth < min_revenue_growth:
                    continue
                
                filtered.append(stock_code)
        
        except Exception as e:
          <CodeFromFile 
  filePath="code_library/005_Chapter5_Candidate_Pool/5.2/code_5_2___init__.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
from dataclasses import dataclass
from typing import List, Dict, Callable
from enum import Enum

class RuleLogic(Enum):
    """规则逻辑"""
    AND = "AND"  # 所有规则必须满足
    OR = "OR"    # 任一规则满足即可
    NOT = "NOT"  # 规则不满足

@dataclass
class FilterRule:
    """筛选规则"""
    name: str                    # 规则名称
    rule_type: str              # 规则类型（basic/mainline/technical/fundamental）
    condition: Callable          # 筛选条件函数
    weight: float = 1.0         # 规则权重
    priority: int = 3            # 优先级（1最高，5最低）
    logic: RuleLogic = RuleLogic.AND  # 逻辑组合

class FilterEngine:
    """筛选规则引擎"""
    
    def __init__(self):
        self.rules: List[FilterRule] = []
        self.basic_filter = BasicFilter()
    
    def add_rule(self, rule: FilterRule):
        """添加规则"""
        self.rules.append(rule)
    
    def filter(
        self,
        stocks: List[str],
        date: str = None,
        rule_combination: Dict = None
    ) -> List[str]:
        """
        执行筛选
        
        Args:
            stocks: 股票代码列表
            date: 日期
            rule_combination: 规则组合配置
                {
                    "logic": "AND",  # 逻辑组合
                    "weights": {...}  # 规则权重
                }
        
        Returns:
            筛选后的股票代码列表
        """
        if rule_combination is None:
            rule_combination = {
                "logic": "AND",
                "weights": {}
            }
        
        # 1. 基础筛选（优先执行）
        filtered = self.basic_filter.filter(stocks, date=date)
        
        # 2. 按优先级排序规则
        sorted_rules = sorted(self.rules, key=lambda r: r.priority)
        
        # 3. 执行规则
        logic = rule_combination.get("logic", "AND")
        weights = rule_combination.get("weights", {})
        
        if logic == "AND":
            # 所有规则必须满足
            for rule in sorted_rules:
                weight = weights.get(rule.name, rule.weight)
                filtered = self._apply_rule(filtered, rule, date=date, weight=weight)
        elif logic == "OR":
            # 任一规则满足即可（合并结果）
            all_results = []
            for rule in sorted_rules:
                weight = weights.get(rule.name, rule.weight)
                result = self._apply_rule(filtered, rule, date=date, weight=weight)
                all_results.extend(result)
            # 去重
            filtered = list(set(all_results))
        
        return filtered
    
    def _apply_rule(
        self,
        stocks: List[str],
        rule: FilterRule,
        date: str = None,
        weight: float = 1.0
    ) -> List[str]:
        """应用单个规则"""
        try:
            result = rule.condition(stocks, date=date)
            return result
        except Exception as e:
            logger.error(f"执行规则 {rule.name} 失败: {e}")
            return stocks

# 使用示例
engine = FilterEngine()

# 添加主线筛选规则
mainline_rule = FilterRule(
    name="主线筛选",
    rule_type="mainline",
    condition=lambda stocks, date: filter_by_mainline_industry(
        stocks, mainline_info, date
    ),
    weight=0.3,
    priority=1
)
engine.add_rule(mainline_rule)

# 添加技术筛选规则
tech_rule = FilterRule(
    name="技术突破",
    rule_type="technical",
    condition=lambda stocks, date: [
        c["code"] for c in filter_technical_breakthrough(stocks, date)
    ],
    weight=0.3,
    priority=2
)
engine.add_rule(tech_rule)

# 添加基本面筛选规则
fundamental_rule = FilterRule(
    name="基本面筛选",
    rule_type="fundamental",
    condition=lambda stocks, date: filter_fundamental(stocks, date),
    weight=0.4,
    priority=3
)
engine.add_rule(fundamental_rule)

# 执行筛选
rule_combination = {
    "logic": "AND",
    "weights": {
        "主线筛选": 0.3,
        "技术突破": 0.3,
        "基本面筛选": 0.4
    }
}

filtered_stocks = engine.filter(
    stocks=all_stocks,
    date="2024-12-12",
    rule_combina<CodeFromFile 
  filePath="code_library/005_Chapter5_Candidate_Pool/5.2/code_5_2_kb.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
# 查询筛选规则相关的知识
results = mcp_client.call_tool(
    "kb.query",
    {
        "query": "筛选规则 技术突破<CodeFromFile 
  filePath="code_library/005_Chapter5_Candidate_Pool/5.2/code_5_2_data_collector.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
# 爬取筛选规则相关网页
content = mcp_client.call_tool(
    "data_collector.crawl_web",
    {
        "url": "https://example.com/filtering-rules",
        "extract_text": True
    }
)
```
-->     self,
        stocks: List[str],
        rule: FilterRule,
        date: str = None,
        weight: float = 1.0
    ) -> List[str]:
        """应用单个规则"""
        try:
            result = rule.condition(stocks, date=date)
            return result
        except Exception as e:
            logger.error(f"执行规则 {rule.name} 失败: {e}")
            return stocks

# 使用示例
engine = FilterEngine()

# 添加主线筛选规则
mainline_rule = FilterRule(
    name="主线筛选",
    rule_type="mainline",
    condition=lambda stocks, date: filter_by_mainline_industry(
        stocks, mainline_info, date
    ),
    weight=0.3,
    priority=1
)
engine.add_rule(mainline_rule)

# 添加技术筛选规则
tech_rule = FilterRule(
    name="技术突破",
    rule_type="technical",
    condition=lambda stocks, date: [
        c["code"] for c in filter_technical_breakthrough(stocks, date)
    ],
    weight=0.3,
    priority=2
)
engine.add_rule(tech_rule)

# 添加基本面筛选规则
fundamental_rule = FilterRule(
    name="基本面筛选",
    rule_type="fundamental",
    condition=lambda stocks, date: filter_fundamental(stocks, date),
    weight=0.4,
    priority=3
)
engine.add_rule(fundamental_rule)

# 执行筛选
rule_combination = {
    "logic": "AND",
    "weights": {
        "主线筛选": 0.3,
        "技术突破": 0.3,
        "基本面筛选": 0.4
    }
}

filtered_stocks = engine.filter(
    stocks=all_stocks,
    date="2024-12-12",
    rule_combination=rule_combination
)
```

<h2 id="section-5-2-6">🛠️ 5.2.6 MCP工具使用</h2>

筛选规则模块与MCP工具集成，支持知识库查询、数据收集等功能。

### KB MCP Server工具

#### kb.query

查询知识库，获取筛选规则相关的文档和代码：

```python
# 查询筛选规则相关的知识
results = mcp_client.call_tool(
    "kb.query",
    {
        "query": "筛选规则 技术突破 基本面筛选 规则组合",
        "collection": "manual_kb",
        "top_k": 5
    }
)
```

### Data Collector MCP工具

#### data_collector.crawl_web

爬取网页内容，收集筛选规则相关的研究资料：

```python
# 爬取筛选规则相关网页
content = mcp_client.call_tool(
    "data_collector.crawl_web",
    {
        "url": "https://example.com/filtering-rules",
        "extract_text": True
    }
)
```

## 🔗 相关章节

- **第2章：数据源模块** - 了解数据获取机制，为筛选规则提供数据支撑
- **第3章：市场分析模块** - 市场分析结果用于筛选条件调整
- **第4章：投资主线识别** - 主线识别结果用于主线筛选
- **第5章：候选池构建** - 了解候选池构建模块的整体设计
- **第5.1节：股票池管理** - 股票池管理为筛选规则提供股票池
- **第6章：因子库** - 筛选后的股票用于因子计算
- **第10章：开发指南** - 了解筛选规则模块的开发规范

## 🔮 总结与展望

<div class="summary-outlook">
  <h3>本节回顾</h3>
  <p>本节系统介绍了筛选规则引擎，包括基础筛选、主线筛选、行业筛选、技术筛选和规则组合逻辑。通过理解筛选规则引擎的实现，帮助开发者掌握如何从股票池中筛选出符合条件的候选股票，为候选池构建提供核心筛选能力。</p>
  
  <h3>下节预告</h3>
  <p>掌握了筛选规则后，下一节将介绍股票评分系统，包括多因子评分、综合评分计算和评分排序方法。通过理解股票评分系统的实现，帮助开发者掌握如何对候选股票进行综合评分。</p>
  
  <a href="/ashare-book6/005_Chapter5_Candidate_Pool/5.3_Stock_Scoring_CN" class="next-section">
    继续学习：5.3 股票评分 →
  </a>
</div>

> **适用版本**: v1.0.0+  
> **最后更新**: 2025-12-12
