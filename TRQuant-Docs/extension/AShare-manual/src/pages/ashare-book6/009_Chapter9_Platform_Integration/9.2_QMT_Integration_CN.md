---
title: "9.2 QMT集成"
description: "深入解析QMT平台集成，包括API封装、交易接口、数据接口、风险控制和策略部署，实现与国金证券QMT平台的完整集成"
lang: "zh-CN"
layout: "/src/layouts/HandbookLayout.astro"
currentBook: "ashare-book6"
updateDate: "2025-12-12"
---

# 🔌 9.2 QMT集成

> **核心摘要：**
> 
> 本节系统介绍TRQuant系统与国金证券QMT平台的集成，包括QMT API封装、交易接口、数据接口、风险控制和策略部署。通过理解QMT集成的核心技术，帮助开发者掌握如何将经过BulletTrade回测验证的策略部署到QMT平台，实现实盘交易和券商环境回测。

QMT集成是TRQuant系统实现实盘交易的另一个关键环节，负责将策略从内部回测环境部署到国金证券的QMT交易平台。

## 📋 章节概览

<script>
function scrollToSection(sectionId) {
  const element = document.getElementById(sectionId);
  if (element) {
    const headerOffset = 100;
    const elementPosition = element.getBoundingClientRect().top;
    const offsetPosition = elementPosition + window.pageYOffset - headerOffset;
    window.scrollTo({
      top: offsetPosition,
      behavior: 'smooth'
    });
  }
}
</script>

<div class="section-overview">
  <div class="section-item" onclick="scrollToSection('section-9-2-1')">
    <h4>🔌 9.2.1 QMT连接</h4>
    <p>连接配置、连接建立、连接管理、国金证券配置</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-9-2-2')">
    <h4>💹 9.2.2 交易接口</h4>
    <p>下单、撤单、查询、订单管理</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-9-2-3')">
    <h4>📊 9.2.3 数据接口</h4>
    <p>行情数据、账户数据、持仓数据</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-9-2-4')">
    <h4>🛡️ 9.2.4 风险控制</h4>
    <p>交易风控、持仓风控、实时监控</p>
  </div>
  <div class="section-item" onclick="scrollToSection('section-9-2-5')">
    <h4>🚀 9.2.5 策略部署</h4>
    <p>代码转换、策略上传、部署验证、与第8章的衔接</p>
  </div>
</div>

## 🎯 学习目标

通过本节学习，您将能够：

- **配置QMT连接**：掌握国金证券QMT平台的连接配置方法
- **使用交易接口**：理解下单、撤单、查询等交易接口的使用
- **获取数据接口**：掌握行情数据、账户数据、持仓数据的获取方法
- **实现风险控制**：理解交易风控和持仓风控的实现机制
- **部署策略**：掌握从BulletTrade回测到QMT部署的完整流程

## 📚 核心概念

### 模块定位

- **工作流位置**：步骤8 - 🚀 实盘交易（券商平台部署）
- **核心职责**：QMT平台集成、策略部署、实盘交易执行
- **服务对象**：经过BulletTrade回测验证的策略

### 技术栈

- **交易平台**：国金证券QMT（迅投量化终端）
- **连接方式**：QMT API（本地进程通信）
- **策略格式**：聚宽风格 → QMT格式转换
- **数据源**：QMT实时行情、账户数据

### 与第8章的衔接

QMT集成是第8章回测验证的后续环节：

1. **第8章**：在韬睿系统内部使用BulletTrade+聚宽数据源进行回测验证
2. **第9章**：将验证通过的策略部署到QMT平台进行券商环境回测和实盘交易

> **详细流程**：请参考[第8章：回测验证](/ashare-book6/008_Chapter8_Backtest/8.1_Backtest_Framework_CN)中的"PTrade/QMT平台部署"部分。

<h2 id="section-9-2-1">🔌 9.2.1 QMT连接</h2>

QMT连接是平台集成的基础，负责建立与国金证券QMT平台的连接。

### 连接配置

<CodeFromFile 
  filePath="code_library/009_Chapter9_Platform_Integration/9.2/code_9_2_validate.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
from dataclasses import dataclass
from typing import Optional

@dataclass
class QMTConfig:
    """QMT连接配置（国金证券）"""
    
    qmt_path: str              # QMT安装路径（如：C:/QMT）
    account_id: str            # 账户ID
    password: Optional[str] = None  # 密码（可选）
    session_id: Optional[int] = None  # 会话ID（可选）
    timeout: int = 30          # 连接超时（秒）
    
    def validate(self) -> Tuple[bool, List[str]]:
        """验证配置"""
        errors = []
        
        if not self.qmt_path:
            errors.append("QMT安装路径不能为空")
        
        if not Path(self.qmt_path).exists():
            errors.append(f"QMT<CodeFromFile 
  filePath="code_library/009_Chapter9_Platform_Integration/9.2/code_9_2___init__.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
import os
import sys
from pathlib import Path
from typing import Dict, Any, Optional

class QMTConnection:
    """QMT连接管理器（国金证券）"""
    
    def __init__(self, config: QMTConfig):
        """
        初始化QMT连接
        
        Args:
            config: QMT配置
        """
        self.config = config
        self.connected = False
        self.session_id: Optional[int] = None
        self.qmt_api = None
    
    def connect(self) -> bool:
        """
        建立QMT连接
        
        Returns:
            bool: 连接是否成功
        """
        try:
            # 验证配置
            valid, errors = self.config.validate()
            if not valid:
                raise ValueError(f"配置错误: {errors}")
            
            # 添加QMT路径到系统路径
            qmt_lib_path = Path(self.config.qmt_path) / "userdata_mini" / "python"
            if qmt_lib_path.exists():
                sys.path.insert(0, str(qmt_lib_path))
            
            # 导入QMT API
            try:
                import xtquant.xtdata as xtdata
                import xtquant.xttrader as xttrader
                self.xtdata = xtdata
                self.xttrader = xttrader
            except ImportError as e:
                raise ImportError(f"无法导入QMT API，请检查QMT安装路径: {e}")
            
            # 连接QMT
            if self._login():
                self.connected = True
                return True
            else:
                return False
                
        except Exception as e:
            logger.error(f"QMT连接失败: {e}")
            return False
    
    def _login(self) -> bool:
        """登录QMT"""
        try:
            # 使用QMT API登录
            # 注意：QMT的登录方式可能因版本而异
            # 这里使用示例代码，实际实现需要根据QMT API文档
            
            # 方式1：使用账户ID和密码登录
            if self.config.password:
                result = self.xttrader.connect(
                    account_id=self.config.account_id,
                    password=self.config.password
                )
            else:
                # 方式2：使用已有会话
                if self.config.session_id:
                    result = self.xttrader.connect(
                        session_id=self.config.session_id
                    )
                else:
                    # 方式3：使用默认连接
                    result = self.xttrader.connect()
            
            if result:
                self.session_id = result.get('session_id') or self.config.session_id
                return True
            else:
                logger.error("QMT登录失败")
                return False
                
        except Exception as e:
            logger.error(f"登录异常: {e}")
            return False
    
    def disconnect(self):
        """断开QMT连接"""
        try:
            if self.connected and self.xttrader:
                # 断开连接
                self.xttrader.disconnect()
        except Exception as e:
            logger.warning(f"断开连接失败: {e}")
        finally:
            self.connected = False
            self.session_id = None
            self.xttrader = <CodeFromFile 
  filePath="code_library/009_Chapter9_Platform_Integration/9.2/code_9_2___init__.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
class QMTConnectionManager:
    """QMT连接管理器"""
    
    def __init__(self):
        self.connections: Dict[str, QMTConnection] = {}
    
    def get_connection(
        self,
        config: QMTConfig
    ) -> QMTConnection:
        """
        获取QMT连接（复用或创建）
        
        Args:
            config: QMT配置
        
        Returns:
            QMTConnection: QMT连接对象
        """
        connection_key = f"{config.qmt_path}:{config.account_id}"
        
        if connection_key in self.connections:
            connection = self.connections[connection_key]
            if connection.is_connected():
                return connection
            else:
                # 连接已断开，重新连接
                del self.connections[connection_key]
        
        # 创建新连接
        connection = QMTConnection(config)
        if connection.connect():
            self.connections[connection_key] = connection
            return connection
        else:
            raise ConnectionError("无法建立QMT连接")
    
    def close_all(self):
        """关闭所有连接"""
        for connection in se<CodeFromFile 
  filePath="code_library/009_Chapter9_Platform_Integration/9.2/code_9_2___init__.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
class QMTTrader:
    """QMT交易接口"""
    
    def __init__(self, connection: QMTConnection):
        """
        初始化交易接口
        
        Args:
            connection: QMT连接
        """
        self.connection = connection
        self.xttrader = connection.xttrader
    
    def submit_order(
        self,
        order_request: OrderRequest
    ) -> Dict[str, Any]:
        """
        提交订单
        
        Args:
            order_request: 订单请求
        
        Returns:
            Dict: 订单结果（包含order_id, status等）
        """
        if not self.connection.is_connected():
            raise ConnectionError("未连接到QMT服务器")
        
        try:
            # 构建订单参数
            order_params = {
                'stock_code': order_request.symbol,
                'order_type': self._convert_order_type(order_request.order_type),
                'order_volume': order_request.quantity,
                'price_type': self._convert_price_type(order_request.order_type),
                'price': order_request.price or 0,
                'strategy_name': 'TRQuant',
                'order_remark': 'TRQuant策略订单'
            }
            
            # 提交订单
            if order_request.side == OrderSide.BUY:
                result = self.xttrader.order_stock(
                    account_id=self.connection.config.account_id,
                    **order_params
                )
            else:
                result = self.xttrader.order_stock(
                    account_id=self.connection.config.account_id,
                    **order_params
                )
            
            if result.get('error_id') == 0:
                return {
                    'order_id': result.get('order_id'),
                    'status': 'submitted',
                    'message': '订单提交成功'
                }
            else:
                raise ValueError(f"订单提交失败: {result.get('error_msg')}")
                
        except Exception as e:
            logger.error(f"下单异常: {e}")
            raise
    
    def _convert_order_type(self, order_type: OrderType) -> int:
        """转换订单类型"""
        # QMT订单类型：23-买入，24-卖出
        # 实际实现需要根据QMT API文档
        if order_type == OrderType.MARKET:
            return 23  # 市价买入
        elif order_type == OrderType.LIMIT:
            return 23  # 限价买入
        else:
            return 23
    
    def _convert_price_type(self, order_type: OrderType) -> int:
        """转换价格类型"""
        # QMT价格类型：1-限价，2-市价
        if order_type == OrderType.MARKET:
            return 2  # 市价
        else:
            return 1  # 限价
    
    def cancel_order(
        self,
        order_id: str
    ) -> Dict[str, Any]:
        """
        撤销订单
        
        Args:
            order_id: 订单ID
        
        Returns:
            Dict: 撤单结果
        """
        if not self.connection.is_connected():
            raise ConnectionError("未连接到QMT服务器")
        
        try:
            # 撤销订单
            result = self.xttrader.cancel_order_stock(
                account_id=self.connection.config.account_id,
                order_id=order_id
            )
            
            if result.get('error_id') == 0:
                return {
                    'order_id': order_id,
                    'status': 'cancelled',
                    'message': '订单撤销成功'
                }
            else:
                raise ValueError(f"订单撤销失败: {result.get('error_msg')}")
                
        except Exception as e:
            logger.error(f"撤单异常: {e}")
            raise
    
    def query_order(
        self,
        order_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        查询订单
        
        Args:
            order_id: 订单ID（可选，不提供则查询所有订单）
        
        Returns:
            List[Dict]: 订单列表
        """
        if not self.connection.is_connected():
            raise ConnectionError("未连接到QMT服务器")
        
        try:
            # 查询订单
            result = self.xttrader.query_stock_orders(
                account_id=self.connection.config.account_id,
                order_id=order_id
            )
            
            if result.get('error_id') == 0:
                return result.get('data', [])
            else:
                raise ValueError(f"订单查询失败: {result.get('error_msg')}")
                
    <CodeFromFile 
  filePath="code_library/009_Chapter9_Platform_Integration/9.2/code_9_2___init__.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
class QMTDataProvider:
    """QMT数据提供者"""
    
    def __init__(self, connection: QMTConnection):
        """
        初始化数据提供者
        
        Args:
            connection: QMT连接
        """
        self.connection = connection
        self.xtdata = connection.xtdata
    
    def get_market_data(
        self,
        symbols: List[str],
        fields: List[str] = None
    ) -> pd.DataFrame:
        """
        获取行情数据
        
        Args:
            symbols: 股票代码列表
            fields: 字段列表（可选，默认所有字段）
        
        Returns:
            pd.DataFrame: 行情数据
        """
        if not self.connection.is_connected():
            raise ConnectionError("未连接到QMT服务器")
        
        try:
            # 获取行情数据
            data = self.xtdata.get_market_data(
                stock_list=symbols,
                period='1d',  # 日线
                count=1
            )
            
            # 转换为DataFrame
            df = pd.DataFrame(data)
            return df
            
        except Exception as e:
            logger.error(f"获取行情数据异常: {e}")
            raise
    
    def get_account_info(self) -> Dict[str, Any]:
        """
        获取账户信息
        
        Returns:
            Dict: 账户信息
        """
        if not self.connection.is_connected():
            raise ConnectionError("未连接到QMT服务器")
        
        try:
            # 获取账户信息
            result = self.connection.xttrader.query_stock_asset(
                account_id=self.connection.config.account_id
            )
            
            if result.get('error_id') == 0:
                return result.get('data', {})
            else:
                raise ValueError(f"账户信息获取失败: {result.get('error_msg')}")
                
        except Exception as e:
            logger.error(f"获取账户信息异常: {e}")
            raise
    
    def get_positions(self) -> pd.DataFrame:
        """
        获取持仓信息
        
        Returns:
            pd.DataFrame: 持仓数据
        """
        if not self.connection.is_connected():
            raise ConnectionError("未连接到QMT服务器")
        
        try:
            # 获取持仓信息
            result = self.connection.xttrader.query_stock_positions(
                account_id=self.connection.config.account_id
            )
            
            if result.get('error_id') == 0:
                data = result.get('data', [])
                return pd.DataFrame(data)
            else:
                raise ValueError(f"持仓信息获取失败: {result.get('error_msg')}")
                
  <CodeFromFile 
  filePath="code_library/009_Chapter9_Platform_Integration/9.2/code_9_2___init__.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
class QMTRiskController:
    """QMT风险控制器"""
    
    def __init__(
        self,
        connection: QMTConnection,
        config: RiskControlConfig
    ):
        """
        初始化风险控制器
        
        Args:
            connection: QMT连接
            config: 风险控制配置
        """
        self.connection = connection
        self.config = config
        self.data_provider = QMTDataProvider(connection)
    
    def check_order_risk(
        self,
        order_request: OrderRequest
    ) -> Tuple[bool, str]:
        """
        检查订单风险
        
        Args:
            order_request: 订单请求
        
        Returns:
            Tuple[bool, str]: (是否通过, 错误信息)
        """
        # 1. 检查价格范围
        if order_request.price:
            if order_request.price < self.config.min_price:
                return False, f"价格低于最小值: {order_request.price}"
            if order_request.price > self.config.max_price:
                return False, f"价格高于最大值: {order_request.price}"
        
        # 2. 获取账户信息
        account_info = self.data_provider.get_account_info()
        total_value = account_info.get('total_asset', 0)
        available_cash = account_info.get('cash', 0)
        
        # 3. 检查单票仓位
        if order_request.side == OrderSide.BUY:
            order_value = order_request.quantity * (order_request.price or 0)
            position_ratio = order_value / total_value if total_value > 0 else 0
            
            if position_ratio > self.config.max_position_ratio:
                return False, f"单票仓位超过限制: {position_ratio:.2%}"
            
            if order_value > available_cash:
                return False, f"可用资金不足: {available_cash:.2f}"
        
        # 4. 检查总仓位
        positions = self.data_provider.get_positions()
        total_position_value = positions['market_value'].sum() if not positions.empty else 0
        total_position_ratio = total_position_value / total_value if total_value > 0 else 0
        
        if total_position_ratio > self.config.max_total_position:
            return False, f"总仓位超过限制: {total_position_ratio:.2%}"
        
        return True, "风险检查通过"
```
-->OrderSide.BUY:
            order_value = order_request.quantity * (order_request.price or 0)
            position_ratio =<CodeFromFile 
  filePath="code_library/009_Chapter9_Platform_Integration/9.2/code_9_2___init__.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
from core.strategy_converter import StrategyCodeConverter

class QMTStrategyDeployer:
    """QMT策略部署器"""
    
    def __init__(self, connection: QMTConnection):
        """
        初始化策略部署器
        
        Args:
            connection: QMT连接
        """
        self.connection = connection
        self.converter = StrategyCodeConverter()
    
    def deploy_strategy(
        self,
        jq_strategy_path: str,
        strategy_name: str
    ) -> Dict[str, Any]:
        """
        部署策略到QMT平台
        
        Args:
            jq_strategy_path: 聚宽风格策略代码路径
            strategy_name: 策略名称
        
        Returns:
            Dict: 部署结果
        """
        # 1. 读取聚宽风格策略代码
        with open(jq_strategy_path, 'r', encoding='utf-8') as f:
            jq_code = f.read()
        
        # 2. 转换为QMT格式
        qmt_code = self.converter.convert_to_qmt(jq_code)
        
        # 3. 保存QMT格式代码
        qmt_strategy_path = jq_strategy_path.replace('.py', '_qmt.py')
        with open(qmt_strategy_path, 'w', encoding='utf-8') as f:
            f.write(qmt_code)
        
        # 4. 上传到QMT平台
        upload_result = self._upload_to_qmt(
            qmt_strategy_path,
            strategy_name
        )
        
        return {
            'strategy_name': strategy_name,
            'qmt_code_path': qmt_strategy_path,
            'upload_result': upload_result
        }
    
    def _upload_to_qmt(
        self,
        strategy_path: str,
        strategy_name: str
    ) -> Dict[str, Any]:
        """上传策略到QMT平台"""
        # QMT策略文件需要放在QMT的策略目录
        qmt_strategies_dir = Path(self.connection.config.qmt_path) / "userdata_mini" / "strategies"
        qmt_strategies_dir.mkdir(parents=True, exist_ok=True)
        
        # 复制策略文件到QMT策略目录
        target_path = qmt_strategies_dir / f"{strategy_name}.py"
        shutil.copy2(strategy_path, ta<CodeFromFile 
  filePath="code_library/009_Chapter9_Platform_Integration/9.2/code_9_2_qmt_deployment_workflow.py"
  language="python"
  showDesignPrinciples="true"
/>

<!-- 原始代码（保留作为备份）：
```python
def qmt_deployment_workflow(
    jq_strategy_path: str,
    strategy_name: str,
    qmt_config: QMTConfig
) -> Dict[str, Any]:
    """
    QMT部署完整工作流
    
    1. 建立QMT连接
    2. 转换策略代码（聚宽风格 → QMT格式）
    3. 上传策略到QMT平台
    4. 验证部署结果
    
    Args:
        jq_strategy_path: 聚宽风格策略代码路径
        strategy_name: 策略名称
        qmt_config: QMT配置
    
    Returns:
        Dict: 部署结果
    """
    # 1. 建立QMT连接
    connection = QMTConnection(qmt_config)
    if not connection.connect():
        raise ConnectionError("无法连接到QMT平台")
    
    try:
        # 2. 创建策略部署器
        deployer = QMTStrategyDeployer(connection)
        
        # 3. 部署策略
        deployment_result = deployer.deploy_strategy(
            jq_strategy_path,
            strategy_name
        )
        
        return {
            'success': True,
            'deployment_result': deployment_result
        }
    finally:
        # 4. 断开连接
        connection.disconnect()
```
-->   # 复制策略文件到QMT策略目录
        target_path = qmt_strategies_dir / f"{strategy_name}.py"
        shutil.copy2(strategy_path, target_path)
        
        return {
            'success': True,
            'strategy_path': str(target_path),
            'message': '策略上传成功'
        }
```

### 完整部署工作流

```python
def qmt_deployment_workflow(
    jq_strategy_path: str,
    strategy_name: str,
    qmt_config: QMTConfig
) -> Dict[str, Any]:
    """
    QMT部署完整工作流
    
    1. 建立QMT连接
    2. 转换策略代码（聚宽风格 → QMT格式）
    3. 上传策略到QMT平台
    4. 验证部署结果
    
    Args:
        jq_strategy_path: 聚宽风格策略代码路径
        strategy_name: 策略名称
        qmt_config: QMT配置
    
    Returns:
        Dict: 部署结果
    """
    # 1. 建立QMT连接
    connection = QMTConnection(qmt_config)
    if not connection.connect():
        raise ConnectionError("无法连接到QMT平台")
    
    try:
        # 2. 创建策略部署器
        deployer = QMTStrategyDeployer(connection)
        
        # 3. 部署策略
        deployment_result = deployer.deploy_strategy(
            jq_strategy_path,
            strategy_name
        )
        
        return {
            'success': True,
            'deployment_result': deployment_result
        }
    finally:
        # 4. 断开连接
        connection.disconnect()
```

## 🔗 相关章节

- **第7章：策略开发**：了解策略开发和测试流程
- **第8章：回测验证**：了解BulletTrade回测和PTrade/QMT部署流程
- **9.1 PTrade集成**：了解PTrade平台集成

## 💡 关键要点

1. **连接管理**：正确配置和建立QMT连接是集成的基础
2. **交易接口**：掌握下单、撤单、查询等交易接口的使用
3. **数据接口**：理解行情数据、账户数据、持仓数据的获取方法
4. **风险控制**：实现交易风控和持仓风控，确保交易安全
5. **策略部署**：掌握从BulletTrade回测到QMT部署的完整流程

## 🔮 总结与展望

<div class="summary-outlook">
  <h3>本节回顾</h3>
  <p>本节系统介绍了QMT平台集成，包括QMT连接、交易接口、数据接口、风险控制和策略部署。通过理解QMT集成的核心技术，帮助开发者掌握如何将经过BulletTrade回测验证的策略部署到国金证券QMT平台，实现实盘交易和券商环境回测。</p>
  
  <h3>下节预告</h3>
  <p>掌握了PTrade和QMT集成后，下一节将介绍实盘反馈与在线再优化，包括实盘反馈机制、在线再优化流程和反馈闭环设计。通过理解实盘反馈的完整机制，帮助开发者掌握如何实现策略的持续优化。</p>
  
  <a href="/ashare-book6/009_Chapter9_Platform_Integration/9.6_Live_Trading_Management_CN" class="next-section">
    继续学习：9.6 实盘交易管理 →
  </a>
</div>

> **适用版本**: v1.0.0+  
> **最后更新**: 2025-12-12
