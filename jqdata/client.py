...
class DataPermission:
    ...
    def __init__(self):
        ...
        self.start_days_offset: Optional[int] = None  # 开始日期相对天数（用于动态计算）
        self.end_days_offset: Optional[int] = None   # 结束日期相对天数（用于动态计算）
...
class JQDataClient:
    ...
    def _detect_permission(self):
        ...
        优先使用 get_account_info() API 获取准确权限信息，
        如果无法获取，再通过尝试获取数据并解析错误信息来确定权限范围
        """
        logger.info("正在检测账号数据权限...")
        
        try:
            # 首先尝试使用 get_account_info() API 获取账号信息
            try:
                account_info = jq.get_account_info()
                logger.debug(f"get_account_info() 返回: {account_info}")
                date_range_start = account_info.get('date_range_start', '')
                date_range_end = account_info.get('date_range_end', '')
                license_type = account_info.get('license', 0)
                logger.debug(f"解析得到: start='{date_range_start}', end='{date_range_end}', license={license_type}")
                
                # 判断账号类型
                is_realtime = license_type >= 2  # license >= 2 通常是正式账号
                
                # 处理日期范围
                if date_range_start == '*' or not date_range_start:
                    # 正式账号，数据起始日期为2005-01-01
                    self.permission.start_date = '2005-01-01'
                else:
                    # 试用账号，使用API返回的开始日期
                    self.permission.start_date = date_range_start.split(' ')[0]
                
                if date_range_end == '*' or not date_range_end or is_realtime:
                    # 实时账号，结束日期为今天
                    self.permission.end_date = date.today().strftime('%Y-%m-%d')
                    self.permission.is_realtime = True
                else:
                    # 历史账号，使用API返回的结束日期
                    self.permission.end_date = date_range_end.split(' ')[0]
                    self.permission.is_realtime = False
                
                self.permission.detected = True
                logger.info(f"✅ 通过 get_account_info() 检测到账号权限: {self.permission}")
                return
                
            except Exception as e:
                logger.debug(f"无法使用 get_account_info() 检测权限: {e}，尝试其他方法...")
            ...
            try:
                ...
                if "账号权限仅能获取" in error_msg:
                    ...
                    if len(dates) >= 2:
                        ...
                        self.permission.start_days_offset = (today_date - start_dt).days
                        self.permission.end_days_offset = (today_date - end_dt).days
                        
                        self.permission.is_realtime = False
                        self.permission.detected = True
                        logger.info(f"✅ 检测到历史账号权限（相对日期）: {self.permission}")
                        return
