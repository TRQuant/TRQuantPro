...
# 轮动参数（优化：更灵活的轮动）
REBALANCE_INTERVAL_WEEKS = 1.5  # 调仓间隔（周，1.5周=约7-8个交易日）
REBALANCE_INTERVAL_DAYS = 8     # 调仓间隔（交易日，约1.5周，加快轮动）
THEME_UPDATE_INTERVAL = 3       # 主题强度更新间隔（交易日，每3天更新一次，更及时）

# 风险控制参数（优化）
STOP_LOSS_SINGLE = -0.10    # 单只股票止损比例（-10%，更严格）
TAKE_PROFIT = 0.30          # 止盈比例（30%，提高止盈目标）
MAX_HOLD_DAYS = 60          # 最大持仓天数（2个月，加快轮动）
MIN_HOLD_DAYS = 3           # 最小持仓天数（3天，允许快速止损）
...
# 选股参数（优化：更严格的筛选）
MIN_STOCK_PRICE = 8.0       # 最小股票价格（元，提高门槛）
MAX_STOCK_PRICE = 200.0     # 最大股票价格（元，避免高价股）
MIN_POSITION_VALUE = 6000   # 最小持仓金额（元，提高单只股票金额）
MIN_TRADING_UNIT = 100      # 最小交易单位（股）
MIN_MARKET_CAP = 50         # 最小市值（亿元，筛选流动性好的股票）
