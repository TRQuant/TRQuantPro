...
# 轮动参数（中低频）
REBALANCE_INTERVAL_WEEKS = 2    # 调仓间隔（周，2周=约10个交易日）
REBALANCE_INTERVAL_DAYS = 10    # 调仓间隔（交易日，约2周）
THEME_UPDATE_INTERVAL = 5       # 主题强度更新间隔（交易日，每周更新一次）

# 风险控制参数
STOP_LOSS_SINGLE = -0.15    # 单只股票止损比例（-15%）
TAKE_PROFIT = 0.25          # 止盈比例（25%）
MAX_HOLD_DAYS = 90          # 最大持仓天数（3个月）
MIN_HOLD_DAYS = 5           # 最小持仓天数（1周）
...
def evaluate_theme_strength(context, theme_key, theme_info, date_str):
    ...
    # 6. 综合评分（加权平均）
    total_score = (
        momentum * 0.30 +          # 动量权重30%
        capital_flow * 0.25 +      # 资金流权重25%
        trend * 0.25 +              # 趋势权重25%
        relative_strength * 0.20    # 相对强度权重20%
    )
...
def select_stocks_from_theme(context, theme_stocks, date_str):
    ...
    for stock in theme_stocks[:100]:  # 限制数量
        try:
            ...
            momentum = (closes[-1] / closes[0] - 1) * 100 if len(closes) > 0 and closes[0] > 0 else 0
            ...
            volume_ratio = (recent_volume / avg_volume - 1) * 100 if avg_volume > 0 else 0
            
            # 计算趋势因子（简单MA）
            ma20 = np.mean(closes[-20:]) if len(closes) >= 20 else np.mean(closes)
            ma5 = np.mean(closes[-5:]) if len(closes) >= 5 else np.mean(closes)
            trend_score = (ma5 / ma20 - 1) * 100 if ma20 > 0 else 0
            
            # 综合评分
            total_score = (
                momentum * 0.40 +      # 动量权重40%
                volume_ratio * 0.30 +   # 成交量权重30%
    ...
    for i, item in enumerate(stock_scores[:5]):
        print(f'  {i+1}. {item["stock"]}: {item["score"]:.2f} '
              f'(动量: {item["momentum"]:.2f}%, 成交量: {item["volume_ratio"]:.2f}%, 趋势: {item["trend"]:.2f}%)')
    ...
    for stock in valid_stocks:
        try:
            ...
            if shares_to_buy < MIN_TRADING_UNIT:
                print(f'[警告] {stock}只能买入{shares_to_buy}股，小于最小交易单位，跳过')
                continue
            
            # 检查实际金额是否超过可用资金
            actual_value = shares_to_buy * current_price
            if actual_value > cash * 1.1:  # 允许10%误差
                print(f'[警告] {stock}目标金额{actual_value:.2f}元超过可用资金{cash:.2f}元，跳过')
                continue
            ...
            print(f'[买入] {stock}: {shares_to_buy}股, 价格: {current_price:.2f}元, 金额: {actual_value:.2f}元')
        except Exception as e:
