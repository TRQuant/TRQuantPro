    ...
    try:
        ...
        for stock in sample_stocks:
            try:
                ...
                # v2.4修复：增加调试信息（前5只股票）
                if debug_count < 5:
                    print(f'[调试] 股票{stock}的板块数据: type={type(stock_blocks)}, value={stock_blocks}')
                    debug_count += 1
                ...
                if isinstance(stock_blocks, (list, tuple)):
                    ...
                    elif len(stock_blocks) > 0:
                        # 如果DataFrame有数据但没有block_name列，尝试使用第一列
                        blocks_list = stock_blocks.iloc[:, 0].tolist()
                elif isinstance(stock_blocks, dict):
                    # 如果是字典，尝试提取板块名称
                    block_name = stock_blocks.get('block_name', '') or stock_blocks.get('name', '') or stock_blocks.get('concept_name', '')
                    if block_name:
                        blocks_list = [block_name]
                
                for block in blocks_list[:3]:  # 每只股票最多取3个板块
                    if isinstance(block, dict):
                        block_name = block.get('block_name', '') or block.get('name', '') or block.get('concept_name', '')
                    else:
                        block_name = str(block).strip()
                    
                    if not block_name or block_name == 'nan' or block_name == 'None':
                        continue
                ...
                if debug_count < 5:
                    print(f'[调试] 处理股票{stock}的板块失败: {e}')
                continue
        
        # v2.4修复：输出调试信息
        print(f'[调试] 识别到 {len(mainline_candidates)} 个候选板块')
        if len(mainline_candidates) > 0:
            top_blocks = sorted(mainline_candidates.items(), key=lambda x: len(x[1]), reverse=True)[:5]
            for block_name, stocks in top_blocks:
                print(f'[调试] 板块 {block_name}: {len(stocks)} 只股票')
        
        # 计算每个板块的动量，选择前N个主线
        mainline_scores = {}
        for block_name, stocks in mainline_candidates.items():
            # v2.4修复：降低板块股票数量要求（从5只降到3只）
            if len(stocks) < 3:  # 板块股票太少，跳过
                continue
            ...
            try:
                ...
                if block_momentums:
                    ...
                    # v2.4修复：如果动量计算失败，使用备选方案（板块股票数量作为评分）
                    # 至少保证有一些主线被识别
                    mainline_scores[block_name] = {
                        'momentum': len(stocks) * 0.001,  # 使用股票数量作为备选评分
                        'stocks': stocks
                    }
            except Exception as e:
                print(f'[调试] 计算板块{block_name}动量失败: {e}')
                continue
        ...
        try:
            ...
            if 'trend_strength' in factor_weights:
                ...
                if trend_score > 0:
                    ...
                    # v2.4修复：如果趋势强度因子失败，使用动量因子替代
                    if 'momentum' in factor_values:
                        factor_values['trend_strength'] = abs(factor_values['momentum']) * 0.5  # 使用动量绝对值的一半
            ...
            if 'industry_momentum' in factor_weights:
                ...
                if industry_score > 0:
                    ...
                    # v2.4修复：如果行业景气度因子失败，使用主线动量因子替代
                    if 'mainline_momentum' in factor_values:
                        factor_values['industry_momentum'] = abs(factor_values['mainline_momentum']) * 0.5
                    elif 'momentum' in factor_values:
                        factor_values['industry_momentum'] = abs(factor_values['momentum']) * 0.3
...
def check_stop_loss(context):
    """
    检查单只股票止损（v2.4优化：考虑最低持仓时间，止损后触发补仓检查）
    ...
    stop_loss_triggered = False  # 标记是否有止损发生
    ...
    if stop_loss_triggered:
        check_rebalance_after_stop_loss(context)


def check_rebalance_after_stop_loss(context):
    """
    止损后补仓检查（v2.4新增：如果持仓比例低于目标，考虑补仓）
    ...
    try:
        current_positions = list(context.portfolio.positions.keys())
        current_num = len(current_positions)
        
        # 如果持仓数量已经达到目标，不需要补仓
        if current_num >= STOCK_NUM:
            return
        ...
        current_position_ratio = current_positions_value / total_value if total_value > 0 else 0
        
        # 获取目标持仓比例
        target_position = g.current_strategy.get('position', MAX_POSITION)
        target_position_ratio = target_position
        
        # 如果持仓比例低于目标，考虑补仓
        if current_position_ratio < target_position_ratio * 0.8:  # 低于目标80%时补仓
            print(f'[补仓检查] 当前持仓: {current_num}只, 目标: {STOCK_NUM}只, 持仓比例: {current_position_ratio*100:.1f}%, 目标: {target_position_ratio*100:.1f}%')
            
            # 计算需要补仓的股票数量
            need_stocks = STOCK_NUM - current_num
            
            # 从候选池中选股（使用简化的选股逻辑）
            # 注意：这里不重新计算主线，使用已有的候选池或全市场
            candidate_stocks = g.mainline_pool if g.mainline_pool else []
            
            if not candidate_stocks and g.all_stocks:
                # 如果主线候选池为空，使用全市场（但排除已持仓的）
                candidate_stocks = [s for s in g.all_stocks[:500] if s not in current_positions]
            
            if candidate_stocks:
                # 简单筛选：价格范围、成交量
                filtered_candidates = []
                for stock in candidate_stocks[:100]:  # 限制检查数量
                    try:
                        price_df = get_price(stock, count=1, fields=['close', 'volume'])
                        if price_df is None or len(price_df) == 0:
                            continue
                        ...
                        if MIN_STOCK_PRICE <= current_price <= MAX_STOCK_PRICE and current_volume > 0:
                            filtered_candidates.append(stock)
                    except:
                        continue
                
                # 选择前N只股票补仓
                stocks_to_buy = filtered_candidates[:need_stocks]
                
                if stocks_to_buy:
                    print(f'[补仓] 准备补仓 {len(stocks_to_buy)} 只股票: {stocks_to_buy[:3]}...')
                    
                    # 计算每只股票的目标金额
                    usable_cash = context.portfolio.cash
                    target_value_per_stock = (usable_cash * 0.9) / len(stocks_to_buy)  # 使用90%的可用资金
                    ...
                    for stock in stocks_to_buy:
                        try:
                            ...
                            if shares_to_buy >= MIN_TRADING_UNIT:
                                ...
                                if actual_value <= usable_cash:
                                    order_target(stock, shares_to_buy)
                                    print(f'[补仓买入] {stock} (股数: {shares_to_buy}股, 金额: {actual_value:.2f}元)')
                                    
                                    g.entry_prices[stock] = current_price
                                    g.hold_days[stock] = 0
                                    usable_cash -= actual_value
                        except Exception as e:
                            print(f'[错误] 补仓买入{stock}失败: {e}')
                            continue
    except Exception as e:
        print(f'[错误] 止损后补仓检查失败: {e}')
