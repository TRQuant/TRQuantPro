...
# -*- coding: utf-8 -*-
"""
Tenbagger Core Strategy V13 - 十倍股核心策略V13（渐进式优化版）
================================================================

基于V9成功因子的渐进式优化：

V9成功因子（保持不变）：
- 选股标准：利润增速>15%，营收增速>10%
- 调仓周期：60天
- 股票池：沪深300
- 严格的熊市保护

V13渐进式优化（小幅调整）：
1. 适度提高止盈阈值：牛市50%→60%，震荡30%→40%，熊市25%→30%
2. 添加跟踪止盈：收益>30%后，回撤20%触发止盈
3. 保护高收益持仓：收益>25%时放宽止损
4. 优化阶段权重：S1阶段股票给予更多持有时间

核心公式：
十倍股 = 早期阶段(S1-S2) × 高成长 × 跟踪止盈 × 动态风控
...
class TenbaggerCoreV13Strategy:
    """十倍股核心策略V13（渐进式优化版）
    ...
    def __init__(self):
        ...
        # V13: 增加max_gain跟踪
        self.positions = {}  # {stock_code: {shares, entry_price, entry_date, stage, stop_loss, max_gain}}
        ...
        # V13新增：跟踪止盈参数（作为传统止盈的补充）
        self.trailing_profit_threshold = 0.35  # 收益>35%后启用跟踪止盈（高于传统止盈阈值）
    ...
    def _check_stop_orders(self, date: str):
        ...
        for stock, pos in list(self.positions.items()):
            ...
            # V13: 跟踪止盈优先（收益>20%后，从最高点回撤15%触发止盈）
            if max_gain > self.trailing_profit_threshold:
                drawdown_from_peak = (max_gain - gain) / (1 + max_gain) if max_gain > 0 else 0
                if drawdown_from_peak > self.trailing_profit_drawdown:
                    self._sell_stock(stock, date, f"跟踪止盈: 最高{max_gain*100:.1f}%回撤到{gain*100:.1f}%")
                    continue
            
            # 止损逻辑
            if gain > self.high_gain_protection:
                # 高收益时：止损设置为成本价（保护本金）
                if current_price <= entry_price:
                    self._sell_stock(stock, date, f"保护止损: 从{max_gain*100:.1f}%回落到{gain*100:.1f}%")
                    continue
            else:
                # 正常止损
            ...
            if current_price >= entry_price * (1 + take_profit_pct):
                self._sell_stock(stock, date, f"止盈: {gain*100:.1f}%")
                continue
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='Tenbagger Core V13 Strategy (Progressive Optimization)')
    ...
    print(f"\n{'='*60}")
    print(f"Tenbagger Core V13 - 十倍股核心策略（渐进式优化版）")
    print(f"{'='*60}")
    print(f"周期: {args.period} ({start_date} ~ {end_date})")
    print(f"核心优化:")
    print(f"  - 止盈阈值: 牛市60%, 震荡40%, 熊市30%（适度提高）")
    print(f"  - 跟踪止盈: 收益>30%后，回撤20%触发止盈")
    print(f"  - 高收益保护: 收益>25%时保护利润")
    print(f"  - 保持V9成功因子: 选股标准、调仓周期")
    print(f"{'='*60}\n")
    
    # 运行回测
    strategy = TenbaggerCoreV13Strategy()
