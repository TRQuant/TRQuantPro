...
class MainlineBasedScanner:
    ...
    def __init__(
        self, jq_client: Optional[JQDataClient] = None, mongo_uri: str = "mongodb://localhost:27017"
    ):
        """
        初始化扫描器
    ...
    def _load_mapped_mainlines_from_mongodb(self) -> List[Dict]:
        ...
        try:
            ...
            if latest:
                ...
                logger.info(
                    f"从MongoDB读取: {latest.get('mapped_count', 0)}/{latest.get('total_count', 0)} 个映射成功"
                )
                return mainlines
    ...
    def _filter_stocks_jqdata(
        self, stocks: List[str], mainline_name: str, period: str, min_score: float, max_stocks: int
        ...
        for i, stock_code in enumerate(stocks_to_process):
            ...
            try:
                ...
                stock = self.scanner._analyze_stock(
                    ...
                    end_date=(
                        self.jq_client.get_available_end_date()
                        if self.jq_client
    ...
    def _get_recommended_features(self, period: str, mapped_mainlines: List[Dict]) -> Dict:
        """获取推荐的特征条件"""
