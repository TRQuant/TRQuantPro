...
class StrongStockScanner:
    ...
    def scan_top_gainers(self, top_n: int = 50) -> ScanResult:
        ...
        try:
            ...
            df = None
            try:
                df = self._fetch_with_retry(lambda: ak.stock_zh_a_spot_em(), "获取涨幅榜")
            except Exception as e:
    ...
    def scan_new_highs(self, period: int = 60, top_n: int = 50) -> ScanResult:
        ...
        try:
            ...
            # 获取实时行情（带重试）
            df = self._fetch_with_retry(lambda: ak.stock_zh_a_spot_em(), "获取创新高数据")
    ...
    def scan_consecutive_up(self, min_days: int = 3, top_n: int = 50) -> ScanResult:
        ...
        try:
            ...
            # 获取连涨连跌数据（带重试）
            df = self._fetch_with_retry(lambda: ak.stock_rank_lxsz_ths(), "获取连涨数据")
    ...
    def scan_volume_breakout(self, volume_ratio_min: float = 2.0, top_n: int = 50) -> ScanResult:
        ...
        try:
            ...
            df = self._fetch_with_retry(lambda: ak.stock_zh_a_spot_em(), "获取放量上涨数据")
