...
class FiveDimensionScorer:
    ...
    def score_theme(
        self, theme_name: str, theme_code: str, stocks: List[str]
    ) -> FiveDimensionScore:
        """
        对单个主题进行五维评分
    ...
    def _score_fundamental(self, stocks: List[str]) -> DimensionScore:
        ...
        desc = (
            f"盈利增速{details.get('profit_growth', 0):.1f}%，ROE {details.get('avg_roe', 0):.1f}%"
        )

        return DimensionScore(
            dimension="fundamental", score=min(20, max(0, score)), details=details, description=desc
        )
    ...
    def _score_technical(self, theme_code: str, stocks: List[str]) -> DimensionScore:
        ...
        return DimensionScore(
            dimension="technical", score=min(20, max(0, score)), details=details, description=desc
        )
    ...
    def _score_capital(self, stocks: List[str]) -> DimensionScore:
        ...
        return DimensionScore(
            dimension="capital", score=min(20, max(0, score)), details=details, description=desc
        )
    ...
    def _score_news(self, theme_name: str) -> DimensionScore:
        ...
        return DimensionScore(
            dimension="news", score=min(20, max(0, score)), details=details, description=desc
        )
    ...
    def _score_position(self, theme_name: str, stocks: List[str]) -> DimensionScore:
        ...
        # 传统/夕阳行业关键词
        DECLINING_INDUSTRIES = ["煤炭", "钢铁", "水泥", "房地产", "纺织", "造纸"]
        ...
        return DimensionScore(
            dimension="position", score=min(20, max(0, score)), details=details, description=desc
        )

    def _get_fundamentals(self, stocks: List[str]) -> Dict:
        """获取财务数据"""
        result = {"avg_profit_growth": 0, "avg_roe": 0, "avg_pe": 30}
        ...
        try:
            ...
            for stock in stocks[:5]:  # 只取前5个
                ...
                try:
                    ...
                    if df is not None and not df.empty:
                        name = (
                            str(df[df["item"] == "股票简称"]["value"].values[0])
                            if len(df[df["item"] == "股票简称"]) > 0
                            else pure_code
                        )

                        leaders.append({"code": stock, "name": name, "reason": "主要成分股"})
                except:
                    leaders.append({"code": stock, "name": pure_code, "reason": "成分股"})
