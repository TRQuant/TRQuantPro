...
class TimeDimensionRepository(ABC):
    ...
    def get_snapshots_range(
        self, collection: str, start_date: str, end_date: str, period: str = None
    ) -> List[Dict]:
        """获取日期范围内的快照"""
        pass
    ...
    def get_change_history(
        self, collection: str, item_id: str = None, limit: int = 100
    ) -> List[Dict]:
        """获取变更历史"""
        pass
...
class MongoDBRepository(TimeDimensionRepository):
    ...
    def _ensure_indexes(self):
        ...
        try:
            ...
            self.db.candidate_pool_snapshots.create_index(
                [("meta.snapshot_date", DESCENDING), ("meta.period", ASCENDING)], unique=True
            )
    ...
    def save_snapshot(self, collection: str, snapshot: Dict) -> bool:
        ...
        try:
            ...
            logger.info(
                f"保存快照成功: {collection}, date={meta.get('snapshot_date')}, period={meta.get('period')}"
            )
            return True
        except Exception as e:
    ...
    def get_snapshot(self, collection: str, date: str, period: str) -> Optional[Dict]:
        ...
        try:
            result = self.db[collection].find_one(
                {"meta.snapshot_date": date, "meta.period": period}
            )
            if result:
                result.pop("_id", None)
            return result
    ...
    def get_latest_snapshot(self, collection: str, period: str = None) -> Optional[Dict]:
        ...
        try:
            ...
            result = self.db[collection].find_one(query, sort=[("meta.created_at", DESCENDING)])
            if result:
                result.pop("_id", None)
            return result
        except Exception as e:
    ...
    def get_snapshots_range(
        ...
        try:
            query = {"meta.snapshot_date": {"$gte": start_date, "$lte": end_date}}
            if period:
                query["meta.period"] = period

            results = list(
                self.db[collection].find(query, sort=[("meta.snapshot_date", DESCENDING)])
            )
    ...
    def get_change_history(
        ...
        try:
            ...
            results = list(
                self.db[collection].find(query, sort=[("timestamp", DESCENDING)], limit=limit)
            )
...
class TimeDimensionManager:
    ...
    def get_candidate_pool_snapshot(
        self, date: str = None, period: Period = Period.MEDIUM
    ...
    def get_latest_candidate_pool(self, period: Period = None) -> Optional[CandidatePoolSnapshot]:
        """获取最新候选池"""
    ...
    def get_candidate_pool_history(
        self, start_date: str, end_date: str, period: Period = None
    ...
    def get_latest_mainline(self, period: Period = None) -> Optional[MainlineSnapshot]:
        """获取最新主线"""
        period_value = period.value if period else None
    ...
    def analyze_rotation(self, days: int = 30, period: Period = Period.MEDIUM) -> Dict:
        """
        分析板块轮动
        ...
        for snapshot in history:
            for ml in snapshot.mainlines:
                ...
                mainline_scores[name].append({"date": snapshot.meta.snapshot_date, "score": score})
