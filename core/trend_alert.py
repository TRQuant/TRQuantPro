...
class TrendAlertManager:
    ...
    def check_trend_change(
        self, current_result: Dict, previous_result: Optional[Dict] = None
    ) -> List[TrendAlert]:
        """
        检测趋势变化
        ...
        try:
            ...
            if curr_phase != prev_phase and prev_phase:
                ...
                alert = TrendAlert(
                    ...
                    details={"prev_phase": prev_phase, "curr_phase": curr_phase},
                )
                alerts.append(alert)
    ...
    def check_capital_flow_alert(
        self, flow_score: float, net_inflow: float
    ) -> Optional[TrendAlert]:
        """检测资金流向异动"""
        if not self.enabled:
            return None
...
def send_desktop_notification(alert: TrendAlert):
    ...
    try:
        ...
        subprocess.run(
            ["notify-send", "-i", icon, "-u", urgency, f"韬睿量化 - {alert.title}", alert.message],
            check=False,
        )
