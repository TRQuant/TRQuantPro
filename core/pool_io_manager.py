...
class PoolIOManager:
    ...
    def export_to_excel(self, stocks: List[Dict], filename: str = None) -> ExportResult:
        ...
        """
        if not stocks:
            return ExportResult(success=False, file_path="", row_count=0, error="没有数据可导出")
        ...
        try:
            ...
            return ExportResult(success=True, file_path=str(file_path), row_count=len(stocks))

        except Exception as e:
            logger.error(f"导出失败: {e}")
            return ExportResult(success=False, file_path="", row_count=0, error=str(e))
    ...
    def compare_versions(self, version_id_1: str, version_id_2: str) -> Optional[VersionDiff]:
        ...
        return VersionDiff(added=added, removed=removed, unchanged=unchanged)
