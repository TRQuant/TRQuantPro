...
class TDXDataReader:
            ...
            for f in day_path.glob("*.day"):
                ...
                if market == "sh":
                    if code.startswith("6"):
                        stocks.append({"code": f"{code}.XSHG", "market": "sh", "name": code})
                else:
                    if code.startswith("0") or code.startswith("3"):
                        stocks.append({"code": f"{code}.XSHE", "market": "sz", "name": code})
    ...
    def import_to_mongodb(self, code: str, data_type: str = "day", overwrite: bool = False) -> int:
        """
        将通达信数据导入MongoDB
            ...
            for record in records:
                ...
                for key in ["date", "datetime"]:
                    if key in record and hasattr(record[key], "strftime"):
                        record[key] = record[key].strftime(
                            "%Y-%m-%d" if key == "date" else "%Y-%m-%d %H:%M:%S"
                        )
