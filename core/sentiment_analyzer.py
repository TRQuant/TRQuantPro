...
class SentimentAnalyzer:
    ...
    def analyze(self) -> SentimentResult:
        ...
        try:
            ...
            logger.info(
                f"😊 情绪分析完成: {result.overall_sentiment.value} ({result.overall_score:.1f})"
            )
    ...
    def add_custom_opinion(
        ...
        sentiment = (
            ...
            if score > 30
            else SentimentLevel.BEARISH if score < -30 else SentimentLevel.NEUTRAL
        )
        ...
        try:
            ...
            try:
                ...
                if df is not None and not df.empty:
                    # 提取热门话题
                    hot_stocks = (
                        df.head(10)["股票名称"].tolist() if "股票名称" in df.columns else []
                    )
    ...
    def _calculate_overall(self, result: SentimentResult):
        ...
        if total_weight > 0:
            result.overall_score = (
                sum(w * s for w, s in zip(weights, scores) if s != 0) / total_weight
            )
        else:
    ...
    def _generate_summary(self, result: SentimentResult) -> str:
        ...
        fg_text = (
            "贪婪"
            if result.fear_greed_index > 70
            else (
                "乐观"
                if result.fear_greed_index > 55
                else (
                    "中性"
                    if result.fear_greed_index > 45
                    else "谨慎" if result.fear_greed_index > 30 else "恐惧"
                )
            )
        )
