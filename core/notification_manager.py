...
class NotificationManager:
    ...
    def save_config(self):
        ...
        try:
            ...
            data = {
                "channels": {
                    channel.value: {"enabled": config.enabled, "config": config.config}
    ...
    def configure_channel(self, channel: NotificationChannel, enabled: bool, config: Dict = None):
        """配置通知渠道"""
        self._channels[channel] = ChannelConfig(enabled=enabled, config=config or {})
        self.save_config()
    ...
    def send(
        ...
        message = NotificationMessage(
            title=title, content=content, type=type, priority=priority, data=data or {}
        )
...
> 类型: {message.type.value}"""
    ...
    def send_trend_alert(
        self, title: str, content: str, priority: NotificationPriority = NotificationPriority.HIGH
    ):
        """发送趋势预警"""
        return self.send(
            title=f"📈 趋势预警: {title}",
    ...
    def send_system_status(
        self, title: str, content: str, priority: NotificationPriority = NotificationPriority.LOW
    ):
        """发送系统状态"""
        return self.send(
