...
class PriceMomentumFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            logger.info(
                f"价格动量因子计算完成(lookback={lookback_days}): 有效值 {result.notna().sum()}/{len(stocks)}"
            )
            return result
...
class ReversalFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            # 获取历史价格
            prices = jq.get_price(
                stocks, end_date=date, count=lookback_days + 5, fields=["close"], panel=False
            )
            ...
            logger.info(
                f"反转因子计算完成(lookback={lookback_days}): 有效值 {result.notna().sum()}/{len(stocks)}"
            )
            return result
...
class RelativeStrengthFactor(BaseFactor):
    ...
    def __init__(
        self, jq_client=None, lookback_days: int = 60, benchmark: str = "000300.XSHG", **kwargs
    ):
        """
        初始化
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            benchmark_prices = jq.get_price(
                self.benchmark, end_date=date, count=lookback_days + 5, fields=["close"]
            )
            ...
            # 计算基准收益
            benchmark_return = (
                benchmark_prices["close"].iloc[-1] / benchmark_prices["close"].iloc[0] - 1
            )
            ...
            logger.info(
                f"相对强弱因子计算完成(lookback={lookback_days}): 有效值 {result.notna().sum()}/{len(stocks)}"
            )
            return result
