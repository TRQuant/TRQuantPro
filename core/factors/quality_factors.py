...
class ROEFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            q = query(indicator.code, indicator.roe).filter(indicator.code.in_(stocks))
...
class GrossMarginFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            q = query(indicator.code, indicator.gross_profit_margin).filter(  # 毛利率
...
class AssetTurnoverFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            # 获取营业收入
            q_income = query(income.code, income.operating_revenue).filter(income.code.in_(stocks))

            df_income = jq.get_fundamentals(q_income, date=date)

            # 获取总资产
            q_balance = query(balance.code, balance.total_assets).filter(balance.code.in_(stocks))
...
class LeverageFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            q = query(balance.code, balance.total_liability, balance.total_assets).filter(
