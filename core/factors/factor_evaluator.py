...
class FactorEvaluator:
    ...
    def calculate_ic(
        self, factor_values: pd.Series, forward_returns: pd.Series, method: str = "spearman"
    ) -> ICResult:
        """
        计算信息系数(IC)
    ...
    def _get_returns(
        self, stocks: List[str], start_date: datetime, end_date: datetime
    ) -> Optional[pd.Series]:
        """获取区间收益率"""
        try:
            import jqdatasdk as jq
    ...
    def group_backtest(
        ...
        for g in range(1, n_groups + 1):
            ...
            if len(cum_ret_series) > 1:
                ...
                group_returns[g] = float(
                    (cum_ret_series[-1] / cum_ret_series[0]) ** (12 / len(period_returns)) - 1
                )
                if period_returns.std() > 0:
                    group_sharpe[g] = float(
                        period_returns.mean() / period_returns.std() * np.sqrt(12)
                    )
                else:
                    group_sharpe[g] = 0.0
            else:
        ...
        is_monotonic = all(x <= y for x, y in zip(returns_list[:-1], returns_list[1:])) or all(
            x >= y for x, y in zip(returns_list[:-1], returns_list[1:])
        )
    ...
    def evaluate_factor(
        ...
        """
        # IC时间序列
        ic_series = self.calculate_ic_series(factor, stocks, start_date, end_date, freq="M")

        # 分组回测
        group_result = self.group_backtest(factor, stocks, start_date, end_date, n_groups=n_groups)
        ...
        return FactorPerformance(
            ...
            long_short_sharpe=group_result.group_sharpe.get(n_groups, 0)
            - group_result.group_sharpe.get(1, 0),
