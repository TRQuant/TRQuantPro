...
class FactorStorage:
    ...
    def _save_factor_values_mongo(
        self, factor_name: str, date: datetime, values: pd.Series, overwrite: bool
    ) -> bool:
        """MongoDB存储因子值"""
        try:
            ...
            # 删除旧数据
            if overwrite:
                collection.delete_many({"factor_name": factor_name, "date": date})
    ...
    def _save_factor_values_file(self, factor_name: str, date_str: str, values: pd.Series) -> bool:
        """文件存储因子值"""
        try:
    ...
    def load_factor_values(
        self, factor_name: str, date: Union[str, datetime], stocks: Optional[List[str]] = None
    ) -> Optional[pd.Series]:
        """
        加载因子值
    ...
    def _load_factor_values_mongo(
        self, factor_name: str, date: datetime, stocks: Optional[List[str]]
    ) -> Optional[pd.Series]:
        """从MongoDB加载因子值"""
        try:
    ...
    def _load_factor_values_file(
        self, factor_name: str, date_str: str, stocks: Optional[List[str]]
    ) -> Optional[pd.Series]:
        """从文件加载因子值"""
        try:
        ...
        if self._connected:
            try:
                self.db.factor_info.update_one(
                    {"factor_name": factor_name}, {"$set": doc}, upsert=True
                )
                return True
            except Exception as e:
        ...
        if self._connected:
            try:
                self.db.factor_performance.update_one(
                    {"factor_name": factor_name, "date": date}, {"$set": doc}, upsert=True
                )
                return True
            except Exception as e:
    ...
    def save_factor_weights(
        ...
        if self._connected:
            try:
                self.db.factor_weights.update_one({"date": date}, {"$set": doc}, upsert=True)
                return True
            except Exception as e:
...
def create_factor_storage(
    mongo_uri: str = "mongodb://localhost:27017/", db_name: str = "trquant_factors"
) -> FactorStorage:
    """创建因子存储实例"""
