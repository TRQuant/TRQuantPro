...
class FactorPoolIntegration:
    ...
    def process_candidate_pool(
        ...
        if factor_weights is None:
            factor_weights = self.DEFAULT_FACTOR_WEIGHTS.get(
                period, self.DEFAULT_FACTOR_WEIGHTS["medium"]
            )

        # 计算因子评分
        factor_scores, factor_details = self._calculate_factor_scores(stocks, date, factor_weights)

        # 融合主线评分
        combined_scores = self._combine_scores(stocks, factor_scores, mainline_scores)
    ...
    def _calculate_factor_scores(
        self, stocks: List[str], date: datetime, factor_weights: Dict[str, float]
        ...
        for factor_name in factor_weights.keys():
            try:
                result = self.factor_manager.calculate_factor(
                    factor_name, stocks, date, winsorize=True, standardize=True
                )
    ...
    def _combine_scores(
        ...
        for stock in stocks:
            ...
            # 加权融合
            combined[stock] = (
                self.factor_weight * factor_score + self.mainline_weight * mainline_score
            )
    ...
    def _get_stock_names(self, stocks: List[str], date: datetime) -> Dict[str, str]:
        """获取股票名称"""
        names = {}
    ...
    def _save_factor_values(self, date: datetime, factor_details: Dict[str, pd.Series]):
        """保存因子值"""
        for factor_name, values in factor_details.items():
            try:
    ...
    def get_factor_radar_data(
        self, stock: str, date: Union[str, datetime], factor_names: Optional[List[str]] = None
    ) -> Dict[str, float]:
        """
        获取股票因子雷达图数据
