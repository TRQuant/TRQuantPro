...
class EPFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            # 处理负PE和极端值
            df["EP"] = np.where(
                (df["pe_ratio"] > 0) & (df["pe_ratio"] < 500), 1.0 / df["pe_ratio"], np.nan
            )
...
class BPFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            # BP = 1/PB
            df["BP"] = np.where(
                (df["pb_ratio"] > 0) & (df["pb_ratio"] < 50), 1.0 / df["pb_ratio"], np.nan
            )
...
class SPFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            q = query(valuation.code, valuation.ps_ratio).filter(valuation.code.in_(stocks))
            ...
            # SP = 1/PS
            df["SP"] = np.where(
                (df["ps_ratio"] > 0) & (df["ps_ratio"] < 100), 1.0 / df["ps_ratio"], np.nan
            )
...
class DividendYieldFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            # 获取股价
            prices = jq.get_price(stocks, end_date=date, count=1, fields=["close"], panel=False)
