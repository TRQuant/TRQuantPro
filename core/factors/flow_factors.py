    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            try:
                # 获取当前持股
                q_current = jq.query(
                    finance.STK_HK_HOLD_INFO.code, finance.STK_HK_HOLD_INFO.share_ratio
                ).filter(
                ...
                if not df_current.empty and not df_past.empty:
                    ...
                    # 计算持股比例变化
                    flow = df_current["share_ratio"] - df_past["share_ratio"].reindex(
                        df_current.index
                    ).fillna(0)

                    result = flow.reindex(stocks)
                    logger.info(
                        f"北向资金因子计算完成: 有效值 {result.notna().sum()}/{len(stocks)}"
                    )
                    return result
            ...
            for stock in stocks:
                ...
                stock_data["signed_money"] = np.where(
                    stock_data["return"] > 0, stock_data["money"], -stock_data["money"]
                )
            ...
            logger.info(
                f"北向资金因子（替代）计算完成: 有效值 {result.notna().sum()}/{len(stocks)}"
            )
            return result
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            for stock in stocks:
                ...
                # 使用价格位置和成交量估算主力行为
                # 高位放量买入 vs 低位放量卖出
                stock_data["price_position"] = (stock_data["close"] - stock_data["low"]) / (
                    stock_data["high"] - stock_data["low"] + 0.0001
                )

                # 价格位置高+成交量大 = 主力买入
                stock_data["main_flow"] = (stock_data["price_position"] - 0.5) * stock_data["money"]
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            try:
                # 当前融资余额
                q_current = jq.query(
                    finance.STK_MT_TOTAL.sec_code, finance.STK_MT_TOTAL.fin_balance
                ).filter(
                ...
                if not df_current.empty and not df_past.empty:
                    ...
                    logger.info(
                        f"融资余额因子计算完成: 有效值 {result.notna().sum()}/{len(stocks)}"
                    )
                    return result
...
class CompositeFlowFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        if self.weights.get("NorthboundFlow", 0) > 0:
            factors["NorthboundFlow"] = self.northbound_factor.calculate(
                stocks, date, **kwargs
            ).values

        if self.weights.get("MainForceFlow", 0) > 0:
            factors["MainForceFlow"] = self.mainforce_factor.calculate(
                stocks, date, **kwargs
            ).values
