...
class SizeFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            q = query(valuation.code, valuation.market_cap).filter(  # 总市值（亿元）
...
class VolatilityFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            # 获取历史价格
            prices = jq.get_price(
                stocks, end_date=date, count=self.lookback_days + 5, fields=["close"], panel=False
            )
...
class BetaFactor(BaseFactor):
    ...
    def __init__(
        self, jq_client=None, lookback_days: int = 120, benchmark: str = "000300.XSHG", **kwargs
    ):
        super().__init__(jq_client=jq_client, **kwargs)
        self.lookback_days = lookback_days
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            amount_data = jq.get_price(
                stocks, end_date=date, count=self.lookback_days, fields=["money"], panel=False
            )
