...
class FactorCorrelationAnalyzer:
    ...
    def calculate_correlation_matrix(self, factor_dict: Dict[str, pd.Series]) -> pd.DataFrame:
        """
        计算因子相关性矩阵
    ...
    def detect_redundant_factors(
        self, factor_dict: Dict[str, pd.Series]
    ...
    def suggest_factor_selection(
        self, factor_dict: Dict[str, pd.Series], ic_values: Optional[Dict[str, float]] = None
    ) -> List[str]:
        """
        建议因子筛选
...
class TurnoverAnalyzer:
    ...
    def calculate_turnover(self, portfolio_history: List[List[str]]) -> List[float]:
        """
        计算历史换手率
        ...
        for i in range(1, len(portfolio_history)):
            ...
            turnover = (
                (len(new_stocks) + len(sold_stocks)) / (2 * total_stocks) if total_stocks > 0 else 0
            )
    ...
    def estimate_transaction_cost(
        self, turnover: float, avg_position_value: float = 1_000_000
    ) -> float:
        """
        估算交易成本
    ...
    def evaluate_factor_tradability(
        self, factor_annual_return: float, avg_turnover: float, periods_per_year: int = 12
    ) -> Dict[str, float]:
        """
        评估因子可交易性
        ...
        return {
            ...
            "cost_ratio": (
                annual_cost / factor_annual_return if factor_annual_return > 0 else np.inf
            ),
        }
