...
class FactorPipeline:
    ...
    def filter_stocks(self, stocks: List[str], date: Union[str, datetime]) -> List[str]:
        ...
        try:
            ...
            logger.info(
                f"股票过滤: {len(stocks)} -> {len(filtered)} "
    ...
    def run_daily(
        ...
        if self.neutralize and factor_results:
            ...
            try:
                factor_results = self.neutralizer.neutralize_batch(
                    factor_results, stocks, date, neutralize_industry=True, neutralize_size=True
                )
                logger.info("因子中性化完成")
            except Exception as e:
        ...
        logger.info(
            f"成功: {self.run_stats['success_factors']}, "
    ...
    def run_monthly_evaluation(
        self, date: Optional[Union[str, datetime]] = None, lookback_months: int = 12
    ) -> Dict[str, Any]:
        """
        运行月度因子评估
        ...
        logger.info(
            f"运行因子月度评估: {start_date.strftime('%Y-%m-%d')} ~ {date.strftime('%Y-%m-%d')}"
        )
        ...
        for factor_name in self.factor_manager.list_factors():
            try:
                ...
                # 评估因子
                performance = self.evaluator.evaluate_factor(factor, stocks, start_date, date)
                ...
                logger.info(
                    f"评估 {factor_name}: IC={performance.ic_mean:.4f}, "
    ...
    def _save_run_log(self, date: datetime):
        ...
        log_data = {
            ...
            "duration_seconds": (
                self.run_stats["end_time"] - self.run_stats["start_time"]
            ).total_seconds(),
...
def main():
    ...
    pipeline = FactorPipeline(
        jq_client=jq_client, stock_pool=args.pool, neutralize=not args.no_neutralize
    )
