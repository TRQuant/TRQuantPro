...
class RevenueGrowthFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            q = query(indicator.code, indicator.inc_revenue_year_on_year).filter(  # 营收同比增速
...
class ProfitGrowthFactor(BaseFactor):
    ...
    def calculate_raw(self, stocks: List[str], date: Union[str, datetime], **kwargs) -> pd.Series:
        ...
        try:
            ...
            q = query(
                indicator.code, indicator.inc_net_profit_year_on_year  # 净利润同比增速
            ).filter(indicator.code.in_(stocks))
...
class CompositeGrowthFactor(BaseFactor):
    ...
    def __init__(self, jq_client=None, weights: Optional[dict] = None, **kwargs):
        ...
        self.weights = weights or {"RevenueGrowth": 0.3, "ProfitGrowth": 0.4, "ROEChange": 0.3}
