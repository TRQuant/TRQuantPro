...
class XtBacktestEngine:
    ...
    def run(self, strategy_func: Callable, on_progress: Callable = None) -> XtBacktestResult:
        """
        运行回测
        ...
        for i, dt in enumerate(dates):
            ...
            # 获取当日数据
            daily_data = {
                stock: df.loc[dt] if dt in df.index else None for stock, df in market_data.items()
            }

            # 执行策略
            signals = strategy_func(self.current_date, daily_data, self.positions.copy(), self.cash)
