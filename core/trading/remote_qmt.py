...
class RemoteQMTClient:
    ...
    def download_history_data(
        self, stock_list: List[str], start_date: str, end_date: str, period: str = "1d"
    ) -> bool:
        """
        下载历史数据
        ...
        try:
            for stock in stock_list:
                xtdata.download_history_data(
                    stock_code=stock, period=period, start_time=start_date, end_time=end_date
                )
    ...
    def get_market_data(
        self, stock_list: List[str], period: str = "1d", start_time: str = "", end_time: str = ""
    ) -> Dict:
        """
        获取行情数据
...
class VMQMTManager:
    ...
    def detect_vm_ip(self) -> Optional[str]:
        ...
        try:
            result = subprocess.run(
                ["virsh", "domifaddr", self.vm_name], capture_output=True, text=True, timeout=5
            )
    ...
    def start_vm(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["virsh", "start", self.vm_name], capture_output=True, text=True, timeout=30
            )
            if result.returncode == 0:
...
def create_remote_qmt_config(host: str = "192.168.1.100", account_id: str = "") -> RemoteQMTConfig:
    """
    创建远程QMT配置
    ...
    Returns:
        RemoteQMTConfig
    """
    return RemoteQMTConfig(host=host, account_id=account_id)
