...
class QMTTrader:
    ...
    def get_orders(self, include_cancelled: bool = False) -> List[Order]:
        ...
        try:
            ...
            for o in orders:
                ...
                if not include_cancelled and status in [
    ...
    def _place_order(
        self, stock_code: str, direction: OrderDirection, price: float, quantity: int
    ) -> Optional[str]:
        """下单"""
        if not self.connected or not self.account:
        ...
        try:
            ...
            order_id = self.trader.order_stock(
                self.account, stock_code, order_type, quantity, price_type, price
            )

            if order_id > 0:
                logger.info(
                    f"下单成功: {stock_code} {direction.value} {quantity}@{price}, ID={order_id}"
                )
                return str(order_id)
            else:
...
class MockQMTTrader:
    ...
    def get_orders(self, include_cancelled: bool = False) -> List[Order]:
        ...
        if not include_cancelled:
            orders = [
                o for o in orders if o.status not in [OrderStatus.CANCELLED, OrderStatus.REJECTED]
            ]
        return orders
