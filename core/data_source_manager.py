...
class DataSourceManager:
    ...
    def _init_jqdata(self):
        ...
        status = DataSourceStatus(
            source_type=DataSourceType.JQDATA, daily_quota=1000000  # 试用版每日100万条
        )
        ...
        try:
            ...
            if jq_config and jq_config.get("username") and jq_config.get("password"):
                ...
                if self._jq_client.authenticate(jq_config["username"], jq_config["password"]):
                    ...
                    logger.info(
                        f"✅ JQData已连接: {status.account_type.value} ({status.start_date} ~ {status.end_date})"
                    )
                else:
    ...
    def _log_status(self):
        ...
        for source_type, status in self._sources.items():
            ...
            logger.info(
                f"   {icon} {source_type.value}: {status.account_type.value if status.is_available else status.error_message}"
            )
        logger.info("=" * 50)
    ...
    def get_price(
        ...
        for source in sources_to_try:
            ...
            try:
                result = self._fetch_from_source(
                    source, code, start_date, end_date, frequency, fields
                )
        ...
        return DataFetchResult(success=False, error="所有数据源均无法获取数据")
    ...
    def _fetch_from_jqdata(
        self, code: str, start_date: str, end_date: str, frequency: str, fields: List[str]
    ) -> DataFetchResult:
        """从JQData获取数据"""
        ...
        try:
            df = self._jq_client.get_price(
                code, start_date=start_date, end_date=end_date, frequency=frequency, fields=fields
            )

            if df is not None and not df.empty:
                return DataFetchResult(success=True, data=df, source=DataSourceType.JQDATA)
        ...
        try:
            ...
            try:
                ...
                if df is not None and not df.empty:
                    ...
                    return DataFetchResult(success=True, data=df, source=DataSourceType.AKSHARE)
        ...
        try:
            ...
            try:
                ...
                if data_list:
                    ...
                    return DataFetchResult(success=True, data=df, source=DataSourceType.BAOSTOCK)
        ...
        try:
            ...
            if data_list:
                ...
                return DataFetchResult(
                    success=True, data=df, source=DataSourceType.LOCAL_CACHE, from_cache=True
                )
        ...
        try:
            ...
            for record in records:
                collection.update_one(
                    {"code": code, "date": record["date"]}, {"$set": record}, upsert=True
                )
