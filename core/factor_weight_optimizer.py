...
class FactorWeightOptimizer:
    ...
    def ic_weighted(
        self, factor_ic_dict: Dict[str, float], min_weight: float = 0.05, max_weight: float = 0.40
    ) -> Dict[str, float]:
        """
        IC加权法确定因子权重
    ...
    def get_scenario_weights(self, scenario: MarketScenario) -> WeightConfig:
        ...
        Returns:
            WeightConfig
        """
        config = self._scenario_library.get(
            scenario, self._scenario_library[MarketScenario.SIDEWAYS]
        )

        return WeightConfig(
            factor_weights=config["weights"], scenario=scenario, description=config["description"]
        )
