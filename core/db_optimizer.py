...
class MongoDBOptimizer:
    ...
    def analyze_slow_queries(self) -> List[Dict]:
        ...
        try:
            ...
            slow_queries = list(
                profile.find({"millis": {"$gt": 100}})  # 超过100ms
    ...
    def cleanup_old_data(
        self, collection_name: str, days: int = 30, date_field: str = "date", dry_run: bool = True
    ) -> int:
        """
        清理旧数据
    ...
    def cleanup_cache(self, max_age_hours: int = 24) -> int:
        ...
        result = self.db["cache"].delete_many({"created_at": {"$lt": cutoff.isoformat()}})
    ...
    def generate_optimization_report(self) -> str:
        ...
        for coll in collections:
            report.append(
                f"  {coll['name']}: {coll['count']}条, {coll['size_mb']:.2f}MB, {coll['index_count']}个索引"
            )
        report.append("")
