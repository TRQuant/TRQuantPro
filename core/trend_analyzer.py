...
class MarketTrendResult:
    ...
    def overall_direction(self) -> TrendDirection:
        """综合趋势方向"""
        avg_score = (
            self.short_term.score * 0.2 + self.medium_term.score * 0.3 + self.long_term.score * 0.5
        )
...
class TrendAnalyzer:
    ...
    def analyze_market(
        ...
        try:
            ...
            logger.info(
                f"市场趋势分析完成: {index_code}, 综合得分={composite:.1f}, 阶段={market_phase}"
            )
            return result
    ...
    def _get_price_data(
        self, code: str, date: Optional[str], days: int = 300
    ...
    def _calc_ma_score(self, df: pd.DataFrame, config: Dict) -> Tuple[float, Dict]:
        ...
        # 均线趋势（斜率）
        ma_fast_slope = (
            (ma_fast.iloc[-1] - ma_fast.iloc[-5]) / ma_fast.iloc[-5] * 100
            if ma_fast.iloc[-5] > 0
            else 0
        )
        ma_slow_slope = (
            (ma_slow.iloc[-1] - ma_slow.iloc[-10]) / ma_slow.iloc[-10] * 100
            if ma_slow.iloc[-10] > 0
            else 0
        )
        ...
        indicators = {
            ...
            "price_vs_ma_fast": (
                (current_close / current_ma_fast - 1) * 100 if current_ma_fast > 0 else 0
            ),
            "price_vs_ma_slow": (
                (current_close / current_ma_slow - 1) * 100 if current_ma_slow > 0 else 0
            ),
        }
    ...
    def _calc_rsi_score(self, df: pd.DataFrame, config: Dict) -> Tuple[float, Dict]:
        ...
        indicators = {"rsi": current_rsi, "rsi_change": current_rsi - prev_rsi}
    ...
    def _calc_bollinger_score(self, df: pd.DataFrame, period: int = 20) -> Tuple[float, Dict]:
        ...
        # 计算位置百分比 (0-100)
        bb_position = (
            (current_close - current_lower) / (current_upper - current_lower) * 100
        ...
        prev_width = (
            ((upper.iloc[-5] - lower.iloc[-5]) / ma.iloc[-5] * 100) if ma.iloc[-5] > 0 else 0
        )
    ...
    def _calc_volume_score(self, df: pd.DataFrame) -> Tuple[float, Dict]:
        ...
        # 价格变化
        price_change = (
            (close.iloc[-1] - close.iloc[-5]) / close.iloc[-5] * 100 if close.iloc[-5] > 0 else 0
        )
    ...
    def _calc_adx_score(self, df: pd.DataFrame, period: int = 14) -> Tuple[float, Dict]:
        ...
        indicators = {
            ...
            "adx_trend": (
                "强趋势" if current_adx > 40 else ("中等趋势" if current_adx > 25 else "震荡")
            ),
    ...
    def _determine_market_phase(
        self, short: TrendSignal, medium: TrendSignal, long: TrendSignal
    ) -> str:
        """
        判断市场阶段（增强版多周期共振分析）
