...
class DataProvider:
    ...
    def __init__(
        self, jq_client=None, cache_dir: Optional[Path] = None, use_disk_cache: bool = True
    ):
        """
        初始化数据提供者
    ...
    def get_price_data(
        ...
        data = self.jq_client.get_price(
            securities=securities, start_date=start_date, end_date=end_date, frequency=frequency
        )
    ...
    def get_current_price(self, security: str, date: datetime) -> Optional[float]:
        ...
        try:
            data = self.get_price_data(
                securities=security, start_date=date, end_date=date, frequency="daily"
            )
            if not data.empty:
                return (
                    data.loc[date, (security, "close")]
                    if isinstance(data.columns, pd.MultiIndex)
                    else data.loc[date, "close"]
                )
        except Exception as e:
    ...
    def get_cache_info(self) -> dict:
        ...
        Returns:
            缓存统计信息
        """
        info = {"memory_cache_size": len(self._cache), "disk_cache_enabled": self.use_disk_cache}
