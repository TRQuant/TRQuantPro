...
class SimpleBacktestEngine:
    ...
    def run(self, stock_scores: Dict[str, pd.DataFrame]) -> BacktestResult:
        ...
        logger.info(
            f"✅ 回测完成: 收益率={metrics.total_return:.2%}, 夏普={metrics.sharpe_ratio:.2f}"
        )
        return result
    ...
    def _get_price_data(self, stocks: List[str]) -> Dict[str, pd.DataFrame]:
        ...
        try:
            ...
            for stock in stocks[:50]:  # 限制数量
                result = manager.get_price(stock, self.config.start_date, self.config.end_date)
    ...
    def _get_benchmark_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            result = manager.get_price(
                self.config.benchmark, self.config.start_date, self.config.end_date
            )
            ...
            if cost > self.cash:
                quantity = (
                    int(
                        self.cash
                        / price
                        / (1 + self.config.commission_rate + self.config.slippage)
                        / 100
                    )
                    * 100
                )
    ...
    def _calculate_metrics(
        self, equity_df: pd.DataFrame, benchmark_df: pd.DataFrame
    ) -> PerformanceMetrics:
        """计算绩效指标"""
        ...
        rf = 0.02 / 252
        sharpe_ratio = (
            (returns.mean() - rf) / returns.std() * np.sqrt(252) if returns.std() > 0 else 0
        )
...
def run_quick_backtest(
    stocks: List[str], start_date: str, end_date: str, initial_capital: float = 1000000.0
) -> BacktestResult:
    """
    快速回测
