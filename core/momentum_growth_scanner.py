...
class MomentumGrowthScanner:
    ...
    def __init__(self, jq_client: Optional[JQDataClient] = None, use_akshare: bool = True):
        """
        初始化扫描器
        ...
        self.period_configs = {
            ...
            "medium": {"lookback_days": 90, "momentum_weight": 0.5, "growth_weight": 0.5},  # 3个月
            "long": {
    ...
    def scan_all_markets(
        self, period: str = "medium", min_score: float = 60.0, max_stocks: int = 100
        ...
        for period_key in ["short", "medium", "long"]:
            ...
            stocks = self._scan_period(
                stock_list=stock_list, period=period_key, min_score=min_score, max_stocks=max_stocks
            )
    ...
    def _scan_period(
        self, stock_list: List[str], period: str, min_score: float, max_stocks: int
    ...
    def _analyze_stock(
        ...
        # 计算动量指标
        momentum_score, momentum_indicators = self._calculate_momentum(price_data, period)

        # 计算成长潜力
        growth_score, growth_indicators = self._calculate_growth(stock_code, end_date)

        # 计算综合得分
        config = self.period_configs.get(period, self.period_configs["medium"])
        composite_score = (
            momentum_score * config["momentum_weight"] + growth_score * config["growth_weight"]
        )
    ...
    def _calculate_momentum(self, price_data: pd.DataFrame, period: str) -> Tuple[float, Dict]:
        """
        计算动量得分
    ...
    def _calculate_growth(self, stock_code: str, date: str) -> Tuple[float, Dict]:
        """
        计算成长潜力得分
