...
class MacroAnalyzer:
    ...
    def analyze(self) -> MacroAnalysisResult:
        ...
        result = MacroAnalysisResult(analysis_date=date.today().strftime("%Y-%m-%d"))
        ...
        try:
            ...
            logger.info(
                f"🌍 宏观分析完成: {result.overall_signal.value} (得分: {result.overall_score:.1f})"
            )
    ...
    def _analyze_gdp(self) -> Optional[MacroIndicator]:
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                prev_value = (
                    self._parse_value(prev.get("今值", prev.get("国内生产总值-绝对值", 0)))
                    if prev is not None
                    else None
                )
    ...
    def _analyze_m2(self) -> Optional[MacroIndicator]:
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                prev_value = (
                    self._parse_value(prev.get("M2-同比增长", prev.get("M2同比", 0)))
                    if prev is not None
                    else None
                )
    ...
    def _analyze_interest_rate(self) -> Optional[MacroIndicator]:
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                prev_value = (
                    self._parse_value(prev.get("1年期LPR", value)) if prev is not None else None
                )
    ...
    def _analyze_exchange_rate(self) -> Optional[MacroIndicator]:
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                prev_value = (
                    self._parse_value(prev_month.get("中行汇买价", prev_month.get("卖出价", value)))
