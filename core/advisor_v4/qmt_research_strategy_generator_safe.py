...
# -*- coding: utf-8 -*-
"""
QMT研究环境策略代码生成器（安全编码版本）
========================================

功能：
1. 生成QMT桌面app研究环境格式的策略代码
2. 使用ContextInfo API，无需连接交易账户
3. 确保UTF-8编码，避免Windows QMT编码问题
4. 所有中文注释使用安全的UTF-8编码
"""
...
class QMTResearchStrategyGeneratorSafe:
    """QMT研究环境策略代码生成器（安全编码版本）"""
    ...
    def generate_strategy_code(self) -> str:
        """
        生成完整的QMT研究环境策略代码（确保UTF-8编码）
        
        Returns:
            策略代码字符串（UTF-8编码）
        """
        # 预计算所有需要格式化的值，避免在f-string中直接使用中文
...
REBALANCE_WEEKDAY = {rebalance_weekday}  # 0=Monday
...
PARTIAL_PROFIT_1 = {partial_profit_1}
PARTIAL_PROFIT_1_RATIO = {partial_profit_1_ratio}
...
MIN_MOMENTUM_20D = {min_momentum_20d}
MAX_REL_POSITION = {max_rel_position}
MIN_MARKET_CAP = {min_market_cap}
MAX_MARKET_CAP = {max_market_cap}
MIN_MOMENTUM_5D = {min_momentum_5d}
MAX_MOMENTUM_5D = {max_momentum_5d}
...
def init(ContextInfo):
    ...
    # Set scheduled tasks
    # QMT research environment uses run_time
    weekday_names = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday']
    weekday_name = weekday_names[REBALANCE_WEEKDAY]
    
    # Weekly rebalancing (Monday 09:35)
    ...
    print(f"   Rebalance: Every {{weekday_cn[REBALANCE_WEEKDAY]}} 09:35")
...
def handlebar(ContextInfo):
    ...
    if ContextInfo.current_dt.weekday() == 0:  # Monday
        index_code = "000300.SH"
        g_stock_pool = ContextInfo.get_stock_list_in_sector(index_code)
        ContextInfo.set_universe(g_stock_pool)
        print(f"[Pre-market] Stock pool updated: {{len(g_stock_pool)}} stocks")
    
    # Risk control check (daily before close)
    check_risk_control(ContextInfo)
    
    # Rebalancing (weekly on specified day)
...
'''
        ...
        return code_template.format(
            ...
            stop_loss_pct=abs(stop_loss) * 100,
            trailing_stop=trailing_stop,
            trailing_stop_trigger=trailing_stop_trigger,
            time_stop_days=time_stop_days,
            partial_profit_1=partial_profit_1,
