...
def get_price_qmt(stocks, start_date=None, end_date=None, count=None, fields=None):
    ...
    try:
        ...
        try:
            ...
            # 如果get_market_data_ex不可用，尝试get_market_data
...
def get_fundamentals_qmt(stocks, date_str, fields=None):
    ...
    try:
        ...
        # 如果get_financial_data不可用，可能需要使用其他方法获取基本面数据
        try:
            ...
            # QMT可能不支持get_financial_data，需要从其他数据源获取
            # 这里返回None，策略需要处理这种情况
            print(f"⚠️ 警告: QMT可能不支持get_financial_data，基本面因子可能无法计算")
            fundamentals = None
