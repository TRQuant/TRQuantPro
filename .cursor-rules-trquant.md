...
### Architect Agent（架构设计智能体）

当你需要设计新模块或系统架构时，请切换到 Architect Agent 角色：
...
### Code Generator Agent（代码生成智能体）

当你需要生成代码时，请切换到 Code Generator Agent 角色：
...
### Quality Checker Agent（规范检查智能体）

当你需要检查代码质量时，请切换到 Quality Checker Agent 角色：
...
- 通用: `python -m py_compile`, `npm run compile`
...
**所有 Python 依赖必须安装到 `extension/venv/` 虚拟环境中**

```bash
# 虚拟环境路径
/home/taotao/dev/QuantTest/TRQuant/extension/venv/
...
```bash
# ✅ 正确方式：使用虚拟环境 pip
extension/venv/bin/pip install <package>

# ❌ 错误方式：直接 pip install（会报 externally-managed-environment 错误）
pip install <package>
```

### 已安装的核心包

- **BulletTrade 0.5.1** - 量化交易系统，兼容聚宽策略
  - CLI: `extension/venv/bin/bullet-trade`
  - 功能：回测、实盘、JupyterLab
...
```bash
# 执行 Python 脚本
extension/venv/bin/python script.py

# 执行 BulletTrade CLI
extension/venv/bin/bullet-trade backtest strategy.py --start 2025-01-01 --end 2025-06-01
extension/venv/bin/bullet-trade live strategy.py --broker qmt
extension/venv/bin/bullet-trade lab
```
...
```bash
# 同步安装所有依赖
extension/venv/bin/pip install -r requirements.txt
```
