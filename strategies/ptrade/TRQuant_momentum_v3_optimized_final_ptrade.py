...
- set_order_cost转换为set_commission(PerTrade)

"""

# -*- coding: utf-8 -*-
"""
TRQuant激进动量策略V3 - BulletTrade改进版
======================================
基于5日+20日双动量的选股策略（改进版）
...
def initialize(context):
    ...
    log.info('=' * 50)
    log.info('策略初始化: TRQuant激进动量V3 (改进版)')
    ...
    if context.trade_count % 20 == 1:
        try:
            ...
            try:
                ...
                context.stock_pool = all_stocks[:300]  # 取前300只
...
def select_stocks(context):
    ...
    if len(stocks) < MAX_STOCKS:
        ...
        if stocks:
            log.info(f'[选股] 返回可用股票: {stocks[:MAX_STOCKS]}')
    ...
    try:
        ...
        score1 = (mom_short * 0.5 + mom_long * 0.5).where(valid1).dropna()
        
        # 方案2: 两个动量都>0（严格）
        valid2 = (mom_short > 0) & (mom_long > 0)
        score2 = (mom_short * 0.5 + mom_long * 0.5).where(valid2).dropna()
...
def fallback_select(context, stocks):
    """兜底选股策略 - 简单市值排序"""
    log.info('[选股] 使用兜底策略: 市值排序')
    try:
        # 获取市值数据
        q = query(valuation.code, valuation.market_cap).filter(
            valuation.code.in_(stocks[:50])
        )
        ...
        if df is not None and not df.empty:
            ...
            log.info(f'[选股] 兜底策略选股: {len(selected)}只')
            return selected
    except Exception as e:
        log.warn(f'[选股] 兜底策略失败: {e}')
...
def filter_stocks(context, stocks):
    ...
    try:
        st = # get_extras("is_st", filtered[:100], ...)  # PTrade不支持，请使用股票名称判断 
