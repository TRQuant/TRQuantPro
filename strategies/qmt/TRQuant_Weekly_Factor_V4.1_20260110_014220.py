#coding:gbk
# TRQuant Weekly Factor Strategy V4.1
# Created: 2026-01-10 01:42:20
# Version History:
...
STRATEGY_MODE = 'fast'       # 'fast' or 'full'
...
SAMPLE_SIZE = 500            # Sample size for 'full' mode (reduce for speed)
...
def get_all_stock_data(ContextInfo, stocks, field, days):
    """
    Get history data for all stocks - OPTIMIZED VERSION
...
def init(ContextInfo):
    """Initialize strategy"""
    print("=" * 70)
    print(f"TRQuant Weekly Factor Strategy V4.0 (Mode: {STRATEGY_MODE})")
    print("=" * 70)
    ...
    # Sample if too many stocks (for performance in full mode)
    if len(ContextInfo.all_stocks) > SAMPLE_SIZE and STRATEGY_MODE == 'full':
        np.random.seed(42)
        ContextInfo.sample_stocks = list(np.random.choice(
            ContextInfo.all_stocks, size=SAMPLE_SIZE, replace=False
        ))
        print(f"[Init] Sampled {SAMPLE_SIZE} stocks for factor calculation")
    else:
...
def handlebar(ContextInfo):
    ...
    print(f"[Data] Retrieved data for {len(close_22)} stocks")
    ...
    print(f"[Filter] {len(stock_scores)} stocks passed filters (score >= {MIN_TOTAL_SCORE})")
