...
#       * Expected: Stock pass rate from 0.23% to 1-2% (5-10x improvement)
#     - OPTIMIZATION: Adjusted rebalance frequency to reduce trading costs
#       * REBALANCE_PERIOD: 5 → 10 trading days (weekly → bi-weekly)
#       * Expected: Trading costs reduced by ~50%, less frequent turnover
#     - Rationale: Backtest showed -6.37% return with only 0.23% stock pass rate
#       * Too strict filters → limited selection → poor diversification
#       * Weekly rebalancing → high transaction costs (7,356.78 / 1,000,000 = 0.79%)
#       * Optimization aims to improve selection diversity and reduce costs
...
# Factor Thresholds (OPTIMIZED V4.8: Relaxed to increase stock selection rate)
# Note: 5%~30% is optimal range, but we use -5% as minimum to allow stocks in weak markets
# The scoring system will penalize low momentum stocks, but won't exclude them completely
# V4.8 Changes: Relaxed thresholds to improve selection diversity (0.23% → 1-2% pass rate expected)
...
def init(ContextInfo):
    ...
    print(f"[Init] Version: V4.8 - Relaxed filters (pass rate 0.23%→1-2%), bi-weekly rebalance (costs -50%)")
