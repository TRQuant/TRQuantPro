...
def _get_cache_file_path(field, days, current_date_str=None):
    """Generate cache file path"""
    cache_dir = get_cache_dir()
    # Include date in cache key to handle different backtest periods
    if current_date_str:
        cache_key = f"{field}_{days}_{current_date_str}"
    else:
...
def _load_from_disk_cache(field, days, current_date_str=None):
    """Load data from disk cache"""
    try:
        cache_file = _get_cache_file_path(field, days, current_date_str)
        if os.path.exists(cache_file):
            ...
            with open(cache_file, 'rb') as f:
                data = pickle.load(f)
            print(f"[Cache] Loaded {field} data from disk cache: {len(data)} stocks")
            return data
    except Exception as e:
        print(f"[Cache Warning] Failed to load from disk: {e}")
    return None

def _save_to_disk_cache(field, days, data, current_date_str=None):
    """Save data to disk cache"""
    try:
        cache_file = _get_cache_file_path(field, days, current_date_str)
        with open(cache_file, 'wb') as f:
            pickle.dump(data, f, protocol=pickle.HIGHEST_PROTOCOL)
        print(f"[Cache] Saved {field} data to disk cache: {len(data)} stocks")
    except Exception as e:
...
def get_all_stock_data(ContextInfo, stocks, field, days, use_disk_cache=True):
    ...
    try:
        ...
        # For historical data (days > 1), use None as date_str to reuse cache across dates
        if use_disk_cache:
            # For historical data, don't pass date_str to reuse cache
            cache_date_str = current_date_str if days == 1 else None
            disk_data = _load_from_disk_cache(field, days, cache_date_str)
            if disk_data is not None:
                # Load into memory cache
                _data_cache[cache_key] = disk_data
                print(f"[Cache] Loaded {field} data from disk cache: {len(disk_data)} stocks")
                return disk_data
        ...
        if use_disk_cache:
            # For historical data, don't pass date_str to allow reuse across dates
            cache_date_str = current_date_str if days == 1 else None
            _save_to_disk_cache(field, days, result, cache_date_str)
...
def init(ContextInfo):
    ...
    print(f"[Init] Version: V4.7 - Added persistent disk cache, optional warmup")
