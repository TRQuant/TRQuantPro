...
def handlebar(ContextInfo):
    ...
    if not stock_scores:
        ...
        print(f"  - Filter thresholds too strict (most common: {max(filter_reject_reasons.items(), key=lambda x: x[1])[0]})")
        print(f"  - Factor calculation may have issues")
        print(f"  - MIN_TOTAL_SCORE ({MIN_TOTAL_SCORE}) too high")
        # If no new stocks, check if should sell existing positions (skip if no current_prices available)
